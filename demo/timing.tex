...
serial mode (np=1) using the np package then in parallel mode (np=2, 3,
4) using the npRmpi package.
...
followed by the ratio (Secs(np)/Secs(1)). Note that
...
\caption{\label{run table}Illustrative timed runs (seconds) with 1
  processor (serial, np package) and 2, 3, and 4 processors (parallel,
  npRmpi package) on an Apple M2 Mac Studio.}
\input timing_all
\end{table}
