  ...
  NPContinuousKernelRowResult *result)
{
  ...
  for(coordinate = segment->coordinate_offset;
    ...
    for(component = 0; component < component_count; ++component) {
      ...
      /* The PDF row is the canonical owner of this validated shape.  A
       * derivative consumer allocates its sibling state before entering this
       * routine, allowing both views to be filled without reconstructing the
       * shape in a second pass.  Level-only consumers leave the pointer NULL. */
      if(context->pdf_row_derivative_component != NULL) {
