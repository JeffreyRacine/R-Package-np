...
/*
  ...
  sweep and write it once. Widths five and six have the symmetric siblings
...
/*
 * A rank owns this complete evaluation row. At widths five and six retain
 * only the unique upper Gram triangle; the caller mirrors it after the
 * collective has assembled the complete row set. Compile-time widths keep
 * the owned-row inner loop specialized without duplicating its lifecycle.
 */
#define NP_LP_DEFINE_SYMMETRIC_OWNED_WIDTH(WIDTH)                           \
static void np_lp_accumulate_owned_row_##WIDTH(                             \
    const NPLPOwnedRowContext *ctx)                                         \
{                                                                            \
  int i, a, b;                                                               \
...
NP_LP_DEFINE_SYMMETRIC_OWNED_WIDTH(5)
NP_LP_DEFINE_SYMMETRIC_OWNED_WIDTH(6)

#undef NP_LP_DEFINE_SYMMETRIC_OWNED_WIDTH
