...
## MPI-specific ownership

npRmpi implements its own native code; it does not call the np package.
The shared files above are mirrored implementation boundaries, not a runtime
dependency on the serial package.

- `MPI2` sections in np.c/jksum.c own rank-local work and the corresponding
  collective completion/failure gates. For example, the LP CV
  `lp_cv_collective_gate` combines local moments/RHS before the common solve.
  The small row helpers themselves do not perform those reductions.
- R orchestration in `../R/session.R`, `../R/np.autodispatch.R` and
  `../R/np.fanout.transaction.R` owns session, dispatch and fan-out policy.
  A native collective is not safe merely because only the master needs a result.
- [Rmpi.c](Rmpi.c), [Rmpi.h](Rmpi.h), [internal.c](internal.c) and
  [conversion.c](conversion.c) form the MPI bridge. [fanout_control.h](fanout_control.h)
  and [fanout_control.c](fanout_control.c) own the private cooperative control
  channel and request-buffer lifetime. See `np_fanout_control_begin`,
  `np_fanout_control_finish` and `np_fanout_control_close`; finishing a
  transaction is not the same as closing its communicator.
- [np_native_safety.c](np_native_safety.c) complements the shared header with
  MPI-aware native safety support. Keep rank participation, count bounds,
  cancellation/quarantine and numerical status propagation separate.
- When modifying these boundaries, inspect both rank-local storage and
  cross-call retained state. Spawn, attach and manual-broadcast/profile routes
  must keep their distinct orchestration and clean teardown contracts.
