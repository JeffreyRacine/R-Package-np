...
static double np_nzd_scalar(double x)
{
  if (x >= 0.0)
    return (x < DBL_EPSILON) ? DBL_EPSILON : x;
  return (x > -DBL_EPSILON) ? -DBL_EPSILON : x;
}

static int np_small_chol_solve(const double *A, const double *b, double *x, int q)
{
  int i, j, k;
  double *L = (double *)R_Calloc((size_t)q * (size_t)q, double);
  double *y = (double *)R_Calloc((size_t)q, double);
...
SEXP C_npindex_ks_lp_fit_fixed_epan2_block(SEXP index_r,
                                     SEXP y_r,
                                     SEXP W_r,
                                     SEXP W_eval_block_r,
                                     SEXP idx_r,
                                     ...
                                     SEXP ridge_r)
{
  SEXP index_s = R_NilValue, y_s = R_NilValue, W_s = R_NilValue;
  SEXP W_eval_s = R_NilValue, idx_s = R_NilValue, ridge_s = R_NilValue;
  SEXP out = R_NilValue, dim_w = R_NilValue, dim_we = R_NilValue;
  const double epan_a = 0.33541019662496845446;
  const double epan_b = 0.067082039324993690892;
  double *index, *y, *W, *W_eval, *ridge, *fit;
  int *idx;
  double h;
  int n, q, m, q_we, nridge;
  int i, jj, j, a, b, r;
  double *A = NULL, *Ar = NULL, *z = NULL, *zr = NULL, *coef = NULL;

  PROTECT(index_s = coerceVector(index_r, REALSXP));
  PROTECT(y_s = coerceVector(y_r, REALSXP));
  PROTECT(W_s = coerceVector(W_r, REALSXP));
  PROTECT(W_eval_s = coerceVector(W_eval_block_r, REALSXP));
  PROTECT(idx_s = coerceVector(idx_r, INTSXP));
  PROTECT(ridge_s = coerceVector(ridge_r, REALSXP));

  dim_w = getAttrib(W_s, R_DimSymbol);
  dim_we = getAttrib(W_eval_s, R_DimSymbol);
  if (LENGTH(dim_w) != 2 || LENGTH(dim_we) != 2)
    error("W and W_eval_block must be matrices");

  n = LENGTH(index_s);
  if (LENGTH(y_s) != n)
    error("index and y must have the same length");
  if (INTEGER(dim_w)[0] != n)
    error("W row count must match index length");

  m = LENGTH(idx_s);
  if (INTEGER(dim_we)[0] != m)
    error("W_eval_block row count must match idx length");
  q = INTEGER(dim_w)[1];
  q_we = INTEGER(dim_we)[1];
  if (q_we != q)
    error("W_eval_block column count must match W");
  if (q <= 0)
    error("W must have at least one column");

  h = asReal(h_r);
  if (!R_FINITE(h) || h <= 0.0)
    error("h must be positive and finite");
  nridge = LENGTH(ridge_s);
  if (nridge <= 0)
    error("ridge must have positive length");

  index = REAL(index_s);
  y = REAL(y_s);
  W = REAL(W_s);
  W_eval = REAL(W_eval_s);
  idx = INTEGER(idx_s);
  ridge = REAL(ridge_s);
  ...
  A = (double *)R_Calloc((size_t)q * (size_t)q, double);
  Ar = (double *)R_Calloc((size_t)q * (size_t)q, double);
  z = (double *)R_Calloc((size_t)q, double);
  zr = (double *)R_Calloc((size_t)q, double);
  coef = (double *)R_Calloc((size_t)q, double);
  if (A == NULL || Ar == NULL || z == NULL || zr == NULL || coef == NULL)
    error("failed to allocate npindex KS LP fixed block buffers");
  ...
  for (jj = 0; jj < m; jj++) {
    ...
    j = idx[jj] - 1;
    if (j < 0 || j >= n)
      error("idx contains an out-of-range row");

    memset(A, 0, (size_t)q * (size_t)q * sizeof(double));
    memset(z, 0, (size_t)q * sizeof(double));
    ...
    for (i = 0; i < n; i++) {
      ...
      XtWy0 += wy;
      for (a = 0; a < q; a++) {
        const double Wa = W[i + n * a];
        z[a] += wy * Wa;
        for (b = a; b < q; b++)
          A[a * q + b] += w * Wa * W[i + n * b];
      }
    }
    ...
    lc_mean = XtWy0 / np_nzd_scalar(sumw);
    fit[jj] = lc_mean;

    for (r = 0; r < nridge; r++) {
      const double rr = ridge[r];
      double alpha;
      double pred = 0.0;
      if (!R_FINITE(rr))
        continue;
      memcpy(Ar, A, (size_t)q * (size_t)q * sizeof(double));
      memcpy(zr, z, (size_t)q * sizeof(double));
      memset(coef, 0, (size_t)q * sizeof(double));
      if (rr > 0.0) {
        for (a = 0; a < q; a++)
          Ar[a * q + a] += rr;
        zr[0] = XtWy0 + rr * XtWy0 / np_nzd_scalar(A[0]);
      }
      if (!np_small_chol_solve(Ar, zr, coef, q))
        continue;
      for (a = 0; a < q; a++) {
        if (!R_FINITE(coef[a])) {
          pred = NA_REAL;
        ...
        pred += W_eval[jj + m * a] * coef[a];
      }
      if (!R_FINITE(pred))
        continue;
      alpha = (rr < 1.0) ? rr : 1.0;
      fit[jj] = (1.0 - alpha) * pred + alpha * lc_mean;
      solved = 1;
      break;
    }
