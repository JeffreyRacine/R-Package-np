...
# Serial guard: npcdens user-style example with plot()
cat("#npcdens: serial user-style plot example ... ")
tryCatch({
  set.seed(42)
  n <- 1000
  x <- rnorm(n)
...
# Serial guard: npreghat parity vs npreg fit
cat("#npreghat: serial fit-operator parity ... ")
tryCatch({
  set.seed(31415)
  n <- 120
  x <- runif(n)
...
cat("#wild: serial plot bootstrap path ... ")
tryCatch({
  set.seed(27182)
  n <- 80
  x <- runif(n)
