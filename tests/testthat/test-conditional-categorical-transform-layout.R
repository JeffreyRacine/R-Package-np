    ...
    for (ky in c("aitchisonaitken", "liracine"))
      for (kx in c("aitchisonaitken", "liracine")) {
        ...
        for (fraction in c(.2, .6)) {
          ...
          prep$myopti[27L] <- 0L
          direct <- conditional_layout_native(prep, raw)
          prep$myopti[27L] <- 1L
          actual <- conditional_layout_native(prep, transformed)
...
test_that("conditional ordinary transformed searches replay on the raw scale", {
  ...
  for (family in c("npcdensbw", "npcdistbw")) {
    ...
    bw <- do.call(getFromNamespace(family, "np"), args)
    ...
    raw <- do.call(getFromNamespace(paste0(".", family, "_eval_only"), "np"),
                   replay)
