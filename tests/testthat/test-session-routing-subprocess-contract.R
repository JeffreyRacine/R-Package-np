...
test_that("session npindex ichimura plain plot-data returns gradient-bearing payload", {
  ...
  res <- run_rscript_subprocess(
    lines = c(
      ...
      "out <- suppressWarnings(plot(bw, xdat=tx, ydat=y, behavior='data', errors='none'))",
...
test_that("session npindex ichimura gradient plot-data returns serial payload contract", {
  ...
  res <- run_rscript_subprocess(
    lines = c(
      ...
      "out <- suppressWarnings(plot(bw, xdat=tx, ydat=y, behavior='data', gradients=TRUE))",
      "stopifnot(length(out) == 1L)",
...
test_that("session npplreg plain fixed lc plot-data completes locally", {
  ...
  res <- run_rscript_subprocess(
    lines = c(
      ...
      "data.out <- suppressWarnings(plot(bw.fix, xdat=tx, ydat=y, zdat=tz, behavior='data', perspective=FALSE, errors='none'))",
      "plot.out <- suppressWarnings(plot(bw.fix, xdat=tx, ydat=y, zdat=tz, behavior='plot-data', perspective=FALSE, errors='none'))",
      "ann.out <- suppressWarnings(plot(bw.ann, xdat=tx, ydat=y, zdat=tz, behavior='data', perspective=FALSE, errors='none'))",
