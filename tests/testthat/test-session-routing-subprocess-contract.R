...
test_that("session npindex residual and evaluation-error branches complete in installed-build subprocess", {
  ...
  res <- run_rscript_subprocess(
    lines = c(
      ...
      "  fit.oos <- npindex(bws=bw, txdat=tx, tydat=y, exdat=ex, eydat=ey, gradients=FALSE, se=TRUE, boot.num=9L)",
      ...
      "  fit.grad <- npindex(bws=bw, txdat=tx, tydat=y, exdat=ex, eydat=ey, gradients=TRUE, se=TRUE, boot.num=9L)",
