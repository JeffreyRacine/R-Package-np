  ...
  for (start in starts) {
    ...
    repeat {
      if (!nzchar(candidate) || candidate %in% seen)
        break
      search_paths <- unique(c(candidate, file.path(candidate, "00_pkg_src", "np")))
      ...
      for (path in search_paths) {
        ...
        if (!is.null(dcf) &&
            nrow(dcf) &&
            identical(unname(dcf[1L, "Package"]), "np")) {
          return(path)
        }
      }
...
np_test_extract_c_function <- function(lines, name) {
...
np_test_compact_source <- function(source) {
  gsub("[[:space:]]+", " ", source)
}
...
np_namespace_hygiene_format <- function(df) {
  if (!nrow(df))
    return("<none>")
  paste(
    sprintf(
