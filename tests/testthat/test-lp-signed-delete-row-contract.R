...
test_that("higher-order fixed and generalized-NN CVLS match signed-WLS oracles", {
  ...
  skip_if_not(
    isTRUE(getFromNamespace(".npRmpi_has_active_slave_pool", "npRmpi")()),
    "numerical oracle requires an active MPI slave pool"
  )
  ...
  for (case in cases) {
    ...
    objective <- getFromNamespace(".npcdensbw_eval_only", "npRmpi")(
      xdat = xdat,
      ydat = ydat,
      bws = bws
    )$objective
...
test_that("remaining conditional higher-order LOO routes match signed-WLS oracles", {
  ...
  for (case in cases) {
    ...
    objective <- if (identical(case$route, "cdens")) {
      ...
      getFromNamespace(".npcdistbw_eval_only", "npRmpi")(
