...
test_that("npreg stores training and evaluation omission metadata separately", {
  local_npreg_quiet({
    ...
    ey <- c(1, 2, NA, 4)
    bw <- np::npregbw(xdat = tx, ydat = ty, bws = 0.4, bandwidth.compute = FALSE)

    fit_eval <- np::npreg(bws = bw, txdat = tx, tydat = ty, exdat = ex, eydat = ey)
    ...
    fit_train <- np::npreg(bws = bw, txdat = tx, tydat = ty)
...
test_that("npreg formula route reports original training and evaluation omissions", {
  local_npreg_quiet({
    ...
    bw <- np::npregbw(y ~ x, data = dat, bws = 0.4, bandwidth.compute = FALSE)
    fit <- np::npreg(bws = bw, data = dat, newdata = newdat, y.eval = TRUE)
...
test_that("npreg residuals reuse the training fit result for training-data fits", {
  local_npreg_quiet({
    ...
    y <- sin(x$x) + 0.1 * x$x
    bw <- np::npregbw(xdat = x, ydat = y, bws = 0.3, bandwidth.compute = FALSE)

    fit <- np::npreg(bws = bw, txdat = x, tydat = y, residuals = TRUE)
    nores <- np::npreg(bws = bw, txdat = x, tydat = y, residuals = FALSE)

    expect_equal(fit$resid, y - fit$mean, tolerance = 1e-12)
    expect_equal(fit$mean, nores$mean, tolerance = 1e-12)

    fn.body <- paste(deparse(body(getFromNamespace("npreg.rbandwidth", "np")),
