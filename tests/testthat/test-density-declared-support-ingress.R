...
test_that("native density support validation precedes preparation", {
  ...
  p<-getFromNamespace(".npudensbw_nomad_native_prepare_args","npRmpi")(dat,b,invalid.penalty="dbmax")
  native<-function(support) .Call("C_np_density_bw_eval",
    p$duno,p$dord,p$dcon,p$mysd,p$myopti,p$myoptd,as.double(b$bw),1L,
    p$penalty_mode,p$penalty_multiplier,p$ckerlb,p$ckerub,support,PACKAGE="npRmpi")
  ...
  valid <- getFromNamespace(".npRmpi_with_local_regression","npRmpi")(native(p$declared.support))
  expect_true(is.finite(valid$fval[1]))
  expect_identical(unname(getFromNamespace(".np_native_categorical_support","npRmpi")(b)),list(as.double(1:4)))
})
