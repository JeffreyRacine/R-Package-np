...
test_that("npplregbw uses coordinated generic bandwidth selection progress", {
  ...
  expect_true(any(grepl("^\\[np\\] Bandwidth selection \\(E\\[y\\|z\\], multistart 2/2, [0-9]+\\.[0-9]%, elapsed [0-9]+\\.[0-9]s, eta [0-9]+\\.[0-9]s\\)$", lines)))
  expect_true(any(grepl("^\\[np\\] Bandwidth selection \\(E\\[x\\|z\\], multistart 1/2, iteration [0-9]+, [0-9]+\\.[0-9]%, elapsed [0-9]+\\.[0-9]s, eta [0-9]+\\.[0-9]s\\)$", lines)))
  expect_true(any(grepl("^\\[np\\] Bandwidth selection \\(E\\[x\\|z\\], multistart 2/2, 100\\.0%, elapsed [0-9]+\\.[0-9]s, eta 0\\.0s\\)$", lines)))
})
