  ...
  for (nslaves in c(1L, 3L)) {
    ...
    result <- npRmpi_run_rscript_subprocess(
      lines = c(
        ...
        "udens_bw <- npudensbw(dat=x,bws=full,bandwidth.compute=FALSE,bwmethod='cv.ml',ckertype='epanechnikov')",
        ...
        "  options(np.tree=tree)",
        "  list(udens=npudensbw(dat=x,bws=udens_bw,bandwidth.compute=TRUE,eval.only=TRUE)$fval,cdens=npRmpi:::.npcdensbw_eval_only(x,y,cdens_bw)$objective,fit=npcdens(bws=cdens_bw,txdat=x,tydat=y,se=TRUE,gradients=TRUE))",
        "}",
        "dense_public <- public(FALSE); tree_public <- public(TRUE)",
        "stopifnot(isTRUE(all.equal(tree_public$udens,dense_public$udens,tolerance=2e-11)))",
