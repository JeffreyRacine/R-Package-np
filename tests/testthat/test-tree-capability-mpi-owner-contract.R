  ...
  for (nslaves in c(1L, 3L)) {
    ...
    result <- npRmpi_run_rscript_subprocess(
      lines = c(
        ...
        "regression_objective_tolerance <- 4*sqrt(.Machine$double.eps)",
        ...
        "  stopifnot(isTRUE(all.equal(tree$objective,dense$objective,tolerance=regression_objective_tolerance)))",
        ...
        "for (result in list(dense_public$fit,tree_public$fit)) {",
        "  stopifnot(inherits(result,'condensity'))",
        "  stopifnot(length(result$condens)==n,length(result$conderr)==n)",
        "  stopifnot(identical(dim(result$congrad),c(n,NCOL(x))))",
        "  stopifnot(identical(dim(result$congerr),c(n,NCOL(x))))",
        "  for (field in c('condens','conderr','congrad','congerr')) stopifnot(all(is.finite(result[[field]])))",
        "}",
