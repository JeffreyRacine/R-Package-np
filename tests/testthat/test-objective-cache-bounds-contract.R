...
test_that("native cache growth is transactional and preserves retained lookup", {
  source_file <- locate_objective_cache_source("np.c")
  skip_if(is.null(source_file), "source file src/np.c unavailable")
  source <- paste(readLines(source_file, warn = FALSE), collapse = "\n")
  ...
  expect_match(source, "safe_free(bwm_nn_cache_keys)", fixed = TRUE)
  expect_match(source, "safe_free(bwm_objective_cache_keys)", fixed = TRUE)
  expect_false(grepl(
