...
test_that("one-sided fixed infinite bounds use the configured span surrogate", {
  ...
  upper_obj_factor1 <- npRmpi:::.npcdensbw_eval_only(dat$x, dat$y, bw_upper_span1)$objective
  lower_obj_factor1 <- npRmpi:::.npcdensbw_eval_only(dat$x, dat$y, bw_lower_span1)$objective
...
test_that("explicit fixed [-Inf, Inf] survives and uses the configured span surrogate", {
  ...
  obj_two_inf_factor1 <- npRmpi:::.npcdensbw_eval_only(dat$x, dat$y, bw_two_inf_span1)$objective
