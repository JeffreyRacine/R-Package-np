test_that("conditional-moment batches equal scalar kernel contractions", {
  ...
  batch <- getFromNamespace(".np_cms_statistics_batch", "npRmpi")(
  ...
  nonpivot <- getFromNamespace(".np_cms_statistics_batch", "npRmpi")(
    xdat = xdat, score = score, bw = bw, fhat = fhat,
  ...
  for (bwtype in c("generalized_nn", "adaptive_nn")) {
    ...
    batch.nn <- getFromNamespace(".np_cms_statistics_batch", "npRmpi")(
...
test_that("bootstrap chunk planners enforce bounded linear workspace", {
  cms.chunk <- getFromNamespace(".np_cms_bootstrap_chunk_size", "npRmpi")
  deneq.chunk <- getFromNamespace(".npdeneq_count_chunk_size", "npRmpi")
...
test_that("density-equality count batches preserve literal resampling", {
  ...
  out <- do.call(npdeneqtest, list(
    x = x, y = y, bw.x = bw.x, bw.y = bw.y,
