...
test_that("conditional public LP CV routes stay thin kernelcv dispatches", {
  ...
  expect_equal(sum(grepl("np_shadow_proof_cv_con_density_ml\\(", lines)), 1L)
  expect_equal(sum(grepl("np_shadow_proof_cv_con_density_ls\\(", lines)), 1L)
  expect_equal(sum(grepl("np_shadow_proof_cv_con_distribution_ls\\(", lines)), 1L)
