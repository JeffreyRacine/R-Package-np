.native_beta_self_map_call <- function(bandwidth.mode,
                                       train.is.eval = 1L,
                                       num.eval = 4L) {
  constant <- function(name) get(name, envir = asNamespace("np"),
                                 inherits = FALSE)
  options <- list(
    num_obs_train = 3L,
...
test_that("beta native self maps reject inconsistent layouts", {
  expect_error(
    .native_beta_self_map_call(getFromNamespace("BW_FIXED", "np")),
    "invalid beta kernel-sum dimensions", fixed = TRUE
  )
  expect_error(
    .native_beta_self_map_call(getFromNamespace("BW_GEN_NN", "np")),
    "invalid beta kernel-sum dimensions", fixed = TRUE
  )
  expect_error(
    .native_beta_self_map_call(
      getFromNamespace("BW_GEN_NN", "np"),
      train.is.eval = 2L,
      num.eval = 3L
    ),
    "invalid beta kernel-sum dimensions", fixed = TRUE
