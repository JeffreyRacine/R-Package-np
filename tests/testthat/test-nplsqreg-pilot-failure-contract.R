...
test_that("the root pilot sends its failure before publishing an error", {
  ...
  captured <- new.env(hash = FALSE, parent = emptyenv())
  captured$sent <- NULL
  captured$sends <- 0L
  ...
  testthat::local_mocked_bindings(
    ...
    mpi.bcast.Robj = function(obj, ...) { captured$sent <- obj; captured$sends <- captured$sends + 1L },
    .package = "npRmpi"
  )
  got <- tryCatch(.nplsqreg_scale_pilot(data.frame(x = 1:4), 1:4,
       dots = list(bwtype = "generalized_nn"), regtype.pilot = "ll"),
       error = identity)
  expect_identical(captured$sends, 1L)
  expect_identical(captured$sent[["condition", exact = TRUE]], original)
