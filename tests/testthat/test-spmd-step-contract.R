...
test_that("SPMD step execution uses collective ACK in session subprocess", {
  ...
  res <- run_spmd_subprocess(
    lines = c(
      ...
      "bcast <- getFromNamespace('.npRmpi_bcast_cmd_expr', 'npRmpi')",
      "env <- mk(opcode='spmd.ping', timeout_class='smoke')",
      "cmd <- substitute({",
      "  run <- get('.npRmpi_spmd_execute_step', envir=asNamespace('npRmpi'), inherits=FALSE)",
      "  run(envelope=ENVELOPE, payload=PAYLOAD, comm=1L, where='spmd-collective')",
      "}, list(ENVELOPE=env, PAYLOAD=list(label='collective-ack')))",
      "ans <- bcast(expr=cmd, comm=1L, caller.execute=TRUE)",
      "stopifnot(is.list(ans), isTRUE(ans$ok), identical(ans$ack$status, 'ACK'))",
...
test_that("SPMD step diverged sequence fails fast with ACK mismatch diagnostics", {
  ...
  res <- run_spmd_subprocess(
    lines = c(
      ...
      "cmd <- substitute({",
      "  run <- get('.npRmpi_spmd_execute_step', envir=asNamespace('npRmpi'), inherits=FALSE)",
      "  run(envelope=ENVELOPE, payload=PAYLOAD, comm=1L, where='spmd-diverge')",
      "}, list(ENVELOPE=env, PAYLOAD=list(label='diverge')))",
      "err <- try(bcast(expr=cmd, comm=1L, caller.execute=TRUE), silent=TRUE)",
