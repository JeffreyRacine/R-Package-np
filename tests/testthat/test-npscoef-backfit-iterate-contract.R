...
test_that("npscoef nearest-neighbor candidate bandwidths reject invalid k values", {
  ...
  disabled <- np:::.npscoef_nn_candidate_bandwidth(
    param = c(0, 1, 2, 100),
    bwtype = "adaptive_nn",
    nobs = 10L
  )
  ...
  enabled <- np:::.npscoef_nn_candidate_bandwidth(
    param = c(0, 1, 2, 100),
    bwtype = "adaptive_nn",
    nobs = 10L
  )
...
test_that("npscoef fast large-h gate rejects unsafe kernel metadata and non-finite evals", {
  ...
  expect_true(np:::.npscoefbw_fast_eligible(safe, fixture$zdat))

  zbad <- fixture$zdat
  zbad$z[1L] <- NA_real_
  expect_false(np:::.npscoefbw_fast_eligible(safe, zbad))
  expect_false(np:::.npscoefbw_fast_eligible(
