test_that("type-II replacement preserves every bandwidth representation", {
  replace <- getFromNamespace(".np_npsig_replace_bandwidth", "npRmpi")
  physical <- getFromNamespace(".np_physical_bandwidth", "npRmpi")
  set.seed(169)
  n <- 32L
...
test_that("type-II scaled tests match physical oracles and leave type I alone", {
  ...
  local_mocked_bindings(.npRmpi_npsig_bootstrap_bw_reselect = stub, .package = "npRmpi")
  saved <- serialize(list(original, physical, selected, selected.physical), NULL)
  seed <- .Random.seed
  # Serial tests cover the complete resampling matrix. Here exercise both
  # changed loops and different-data pairwise routing under a real pool,
  # without multiplying unchanged collective-startup work twelvefold.
  cells <- list(list(index = 2L, joint = FALSE),
                list(index = 1:2, joint = TRUE))
  for (which.cell in seq_along(cells)) {
      cell <- cells[[which.cell]]
      boot.method <- c("iid", "pairwise")[[which.cell]]
      state$selected <- selected
      state$count <- 0L
