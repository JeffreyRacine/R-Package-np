test_that("regression k1 capability is narrow and R-owned", {
  ...
  capability <- getFromNamespace("npRegressionNnCapability", "npRmpi")
  lower <- getFromNamespace("npRegressionNnLowerBound", "npRmpi")
  prepare <- getFromNamespace(".npregbw_prepared_args", "npRmpi")
  setup_fun <- getFromNamespace(".npregbw_nomad_bw_setup", "npRmpi")
  bounds_fun <- getFromNamespace(".npregbw_nomad_bw_bounds", "npRmpi")
  search_lower <- getFromNamespace("npRegressionNnSearchLowerBound", "npRmpi")

  density.bw <- mpi_local(npudensbw(
  ...
  expect_error(
    mpi_local(nplsqregbw(
      xdat = x, ydat = y, scale = rep.int(1, nrow(x)),
...
test_that("k1 zero radii retain objective-penalty and fit-error symmetry", {
  ...
  expect_identical(
    as.numeric(npRmpi:::.npregbw_eval_only(x, y, bw)$objective),
    10 * mean((y - mean(y))^2)
  )
  expect_error(mpi_local(npreg(bws = bw)), "zero literal radius", fixed = TRUE)
  expect_error(
