...
test_that("npsigtest no longer uses legacy console helpers", {
  ...
  expect_true(grepl("if (joint) \"Testing joint significance\" else paste(\"Testing\", tested.names[[1L]])", src, fixed = TRUE))
  expect_true(grepl("progress$label <- paste(\"Testing\", tested.names[[ii]])", src, fixed = TRUE))
  expect_true(grepl(".np_npsig_progress_promote(", src, fixed = TRUE))
