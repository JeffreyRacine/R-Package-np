...
test_that("plbandwidth bootstrap non-coef path avoids repeated npplreg refits", {
  skip_if_not_installed("np")
  ...
  x <- runif(n)
  z <- runif(n)
  y <- 1 + 0.8 * x + sin(2 * pi * z) + rnorm(n, sd = 0.1)
  xdat <- data.frame(x = x)
  zdat <- data.frame(z = z)
  bw <- npplregbw(xdat = xdat, ydat = y, zdat = zdat, regtype = "lc", nmulti = 1)

  np.ns <- asNamespace("np")
  assign(".npplreg_trace_count", 0L, envir = .GlobalEnv)
  trace(
    what = "npplreg",
    where = np.ns,
    tracer = quote(assign(".npplreg_trace_count",
                          get(".npplreg_trace_count", envir = .GlobalEnv) + 1L,
                          envir = .GlobalEnv)),
    print = FALSE
  )
  on.exit({
    untrace("npplreg", where = np.ns)
    if (exists(".npplreg_trace_count", envir = .GlobalEnv, inherits = FALSE))
      rm(".npplreg_trace_count", envir = .GlobalEnv)
  }, add = TRUE)
  ...
  expect_type(out, "list")
  expect_true(length(out) > 0)
  expect_lte(get(".npplreg_trace_count", envir = .GlobalEnv), 1L)
})

test_that("sibandwidth bootstrap plot mode avoids npindex refits", {
  skip_if_not_installed("np")
  ...
  bw <- npindexbw(xdat = xdat, ydat = y, method = "ichimura", nmulti = 1)

  np.ns <- asNamespace("np")
  assign(".npindex_trace_count", 0L, envir = .GlobalEnv)
  trace(
    what = "npindex",
    where = np.ns,
    tracer = quote(assign(".npindex_trace_count",
                          get(".npindex_trace_count", envir = .GlobalEnv) + 1L,
                          envir = .GlobalEnv)),
    print = FALSE
  )
  on.exit({
    untrace("npindex", where = np.ns)
    if (exists(".npindex_trace_count", envir = .GlobalEnv, inherits = FALSE))
      rm(".npindex_trace_count", envir = .GlobalEnv)
  }, add = TRUE)
  ...
  expect_identical(get(".npindex_trace_count", envir = .GlobalEnv), 0L)
})
