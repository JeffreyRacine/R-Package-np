test_that("automatic fold recovery separates incumbent and effective domains", {
  schedule <- npRmpi:::.np_nn_ordinary_schedule(c(23, .3, 5), c(1L, 3L),
    caps = c(22, 22), incumbent.caps = 23)
  expect_identical(schedule, list(c(22, .3, 5), c(22, .3, 10),
                                 c(22, .3, 20), c(22, .3, 22)))
  expect_length(npRmpi:::.np_nn_ordinary_schedule(c(24, .3, 5), c(1L, 3L),
    caps = c(22, 22), incumbent.caps = 23), 0L)
  expect_length(npRmpi:::.np_nn_ordinary_schedule(23, 1L, caps = 22), 0L)
  seen <- list()
  z <- npRmpi:::.np_nn_find_raw_valid_start(c(23, .3), 1L, 22,
    raw.eval = function(x) { seen[[length(seen)+1L]] <<- x; .25 },
    incumbent.caps = 23)
  expect_identical(seen, list(c(22, .3)))
  expect_true(z$found)
  expect_identical(z$evaluations, 1L)
  expect_error(npRmpi:::.np_nn_ordinary_schedule(2, 1L, 4, incumbent.caps = 3),
...
test_that("distribution recovery respects the deleted-fit cap and raw objective", {
  ...
  raw <- function(b) npRmpi:::npudistbw.dbandwidth(dat = x, bws = b,
