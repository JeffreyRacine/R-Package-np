...
test_that("phase1 npcdensbw cv.ls fixed lc matches the frozen public baseline", {
  ...
  # Canonical CVLS uses the same delete-one X smoother in both loss terms.
  expect_equal(bw.lc$fval, 3.0853740158379566, tolerance = 1e-10)
})
...
test_that("phase1 npcdensbw cv.ls fixed keeps ll on canonical lp degree-1 glp", {
  ...
  expect_equal(bw.ll$fval, 3.0508772436523763, tolerance = 1e-8)
  expect_equal(bw.lp$fval, 3.0508772436523763, tolerance = 1e-8)
