...
test_that("native NOMAD C callback path does not call R API or longjmp helpers", {
  ...
  callback_path <- c(
    "bwmfunc_wrapper",
    "np_regression_prepared_native_decode_bw",
    "np_regression_prepared_eval_native_raw",
    ...
    "np_regression_prepared_native_search_callback",
  ...
  expect_false(grepl(
    "\\bnp_distribution_conditional_nomad_native_eval_once\\b",
    source_text,
    perl = TRUE
  ))
  expect_false(grepl(
