...
test_that("conditional LP tile planner owns a truthful total budget", {
  ...
  expect_match(
    src,
    "np_conditional_lp_cvls_block_size_with_fixed(num_train",
    fixed = TRUE
  )
  expect_match(
    src,
...
test_that("rank-owned tile routes agree on allocation capability", {
  src_file <- locate_cvls_workspace_source("jksum.c")
  skip_if(is.null(src_file), "source file src/jksum.c unavailable")
  src <- paste(readLines(src_file, warn = FALSE), collapse = "\n")

  expect_match(src, "np_cvls_workspace_collective_status(", fixed = TRUE)
  expect_match(
    src,
  ...
  expect_gte(lengths(regmatches(
    src,
    gregexpr(
      "if\\(use_parallel_blocks\\)[[:space:]]+workspace_status = np_cvls_workspace_collective_status",
      src,
      perl = TRUE
    )
  )), 3L)
