...
test_that("npplreg direct bws fit emits known-total fit progress", {
  ...
  expect_true(any(grepl(
    "^\\[npRmpi\\] Fitting partially linear regression 1/3 \\(33\\.3%, elapsed [0-9]+\\.[0-9]s, eta [0-9]+\\.[0-9]s\\): E\\[y\\|z\\]$",
    lines
  )))
  expect_true(any(grepl(
    "^\\[npRmpi\\] Fitting partially linear regression 2/3 \\(66\\.7%, elapsed [0-9]+\\.[0-9]s, eta [0-9]+\\.[0-9]s\\): E\\[x\\|z\\]$",
    lines
  )))
  expect_true(any(grepl(
    "^\\[npRmpi\\] Fitting partially linear regression 3/3 \\(100\\.0%, elapsed [0-9]+\\.[0-9]s, eta 0\\.0s\\): final partially linear solve$",
    lines
  )))
})
