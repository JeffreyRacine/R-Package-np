.ann_cdf_grid_objective <- function(dat, state, grid = NULL) {
  getFromNamespace("npudistbw.dbandwidth", "npRmpi")(
...
.ann_cdf_grid_refit <- function(dat, state, grid = NULL) {
  # Reference fits do not need a separate fan-out per deleted observation.
  # The objective under test remains MPI-dispatched; collective ownership is
  # additionally covered by the dedicated native-rank sentinel.
  previous <- options(npRmpi.autodispatch = FALSE)
