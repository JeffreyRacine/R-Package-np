test_that("fast-path option validators reject silent repair", {
  ...
  expect_true(npRmpi:::npLogicalOption("np.largeh", TRUE))
  expect_false(npRmpi:::npLogicalOption("np.largelambda", TRUE))
  expect_true(npRmpi:::npExtendedNnEnabled())
  expect_equal(npRmpi:::npLargehRelTol(), 0.002)
  expect_equal(npRmpi:::npDiscUpperRelTol(), 0.02)

  withr::local_options(np.largeh = 1)
  expect_error(npRmpi:::npLogicalOption("np.largeh", TRUE), "TRUE or FALSE")

  withr::local_options(np.largeh = NA)
  expect_error(npRmpi:::npLogicalOption("np.largeh", TRUE), "TRUE or FALSE")

  withr::local_options(np.largeh.rel.tol = -1)
  expect_error(npRmpi:::npLargehRelTol(), "finite numeric scalar")

  withr::local_options(np.disc.upper.rel.tol = 0.75)
  expect_error(npRmpi:::npDiscUpperRelTol(), "finite numeric scalar")
})
