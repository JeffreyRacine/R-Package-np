test_that("complete LP term counts are basis-neutral", {
  ...
  for (fixture in fixtures) {
    actual <- npRmpi:::npLpCompleteTermStatus(
      fixture$basis,
      fixture$degree,
...
test_that("LP statistical rank and native capacity are distinct", {
  ...
  for (basis in names(degree)) {
    expect_identical(
      npRmpi:::npLpCompleteTermStatus(basis, degree[[basis]], 5L)$status,
      "rank_inadmissible"
    )
    expect_identical(
      npRmpi:::npLpCompleteTermStatus(basis, degree[[basis]], 6L)$status,
      "rank_inadmissible"
    )
    expect_identical(
      npRmpi:::npLpCompleteTermStatus(basis, degree[[basis]], 7L)$status,
      "valid"
    )
  }

  expect_identical(
    npRmpi:::npLpCompleteTermStatus(
