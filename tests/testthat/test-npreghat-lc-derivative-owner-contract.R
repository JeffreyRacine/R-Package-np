npreghat_lc_derivative_owner_case <- function() {
  ...
  lines <- c(
    ...
    "set.seed(7331)",
    "n <- 90L",
    "txc <- data.frame(x = sort(runif(n)))",
    "yc <- sin(5 * txc$x) + rnorm(n, sd = 0.05)",
    "for (bt in c('fixed', 'generalized_nn')) {",
    "  bv <- if (identical(bt, 'fixed')) 0.13 else 11",
    "  bwc <- npregbw(xdat = txc, ydat = yc, regtype = 'lc', bwtype = bt, ckertype = 'epanechnikov', bws = bv, bandwidth.compute = FALSE)",
    "  options(np.tree = FALSE)",
    "  dense <- npreghat(bws = bwc, txdat = txc, s = 1L)",
    "  options(np.tree = TRUE)",
    "  tree <- npreghat(bws = bwc, txdat = txc, s = 1L)",
    "  fitc <- npreg(bws = bwc, gradients = TRUE)",
    "  stopifnot(isTRUE(all.equal(tree, dense, tolerance = 1e-14)))",
    "  stopifnot(isTRUE(all.equal(drop(tree %*% yc), fitc$grad[, 1L], tolerance = 1e-12)))",
    "}",
    "set.seed(20260630)",
    "n <- 36L",
    "idx <- seq_len(n)",
