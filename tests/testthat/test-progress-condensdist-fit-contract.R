...
run_condensdist_session_progress_contract <- function(timeout = 120L) {
  ...
  res <- npRmpi_run_rscript_subprocess(
    lines = c(
      ...
      "fit.reg <- npreg(y ~ x, data = dat, nomad = TRUE, degree.max = 1L, nmulti = 0L)",
      "stopifnot(inherits(fit.reg, 'npregression'))",
      "cat('CASE_DONE npreg_preface\\n')",
      "cat('CASE_START npcdens\\n')",
      "fit.cdens <- npcdens(y ~ x, data = dat, cykerbound = 'range', nomad = TRUE, degree.max = 1L, nmulti = 0L)",
      "stopifnot(inherits(fit.cdens, 'condensity'))",
      "cat('CASE_DONE npcdens\\n')",
      "cat('CASE_START npcdist\\n')",
      "fit.cdist <- npcdist(y ~ x, data = dat, nomad = TRUE, degree.max = 1L, nmulti = 0L, ngrid = 7L)",
...
run_condensdist_profile_progress_contract <- function(timeout = 120L) {
  ...
  writeLines(c(
    ...
    "cat('CASE_START npreg_preface\\n')",
    "fit.reg <- mpi.bcast.cmd(npreg(y ~ x, data = dat, nomad = TRUE, degree.max = 1L, nmulti = 0L), caller.execute = TRUE)",
    "stopifnot(inherits(fit.reg, 'npregression'))",
    "cat('CASE_DONE npreg_preface\\n')",
    "cat('CASE_START npcdens\\n')",
    "fit.cdens <- mpi.bcast.cmd(npcdens(y ~ x, data = dat, cykerbound = 'range', nomad = TRUE, degree.max = 1L, nmulti = 0L), caller.execute = TRUE)",
    "stopifnot(inherits(fit.cdens, 'condensity'))",
    "cat('CASE_DONE npcdens\\n')",
