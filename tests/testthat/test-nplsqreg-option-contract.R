test_that("session nplsqreg honors estimator bandwidth options separately from pilot options", {
  ...
  res <- npRmpi_run_rscript_subprocess(
    lines = c(
      ...
      "stopifnot(identical(fit.plot$bws$regtype.engine, 'lc'))",
      "stopifnot(identical(fit.plot$bws$basis.engine, 'glp'))",
      "stopifnot(identical(fit.plot$bws$degree.engine, 0L))",
      "stopifnot(identical(fit.plot$bws$bernstein.basis.engine, FALSE))",
