...
test_that("CVLS Y convolution supertile is memory bounded and topology isolated", {
...
test_that("CVLS Y convolution supertile preserves per-block consumer order", {
  src_file <- locate_yconv_supertile_source()
  skip_if(is.null(src_file), "source file src/jksum.c unavailable in this test context")
  lines <- readLines(src_file, warn = FALSE)
  body <- yconv_source_body(
...
test_that("CVLS Y convolution supertile dispatch leaves excluded routes intact", {
  src_file <- locate_yconv_supertile_source()
  skip_if(is.null(src_file), "source file src/jksum.c unavailable in this test context")
  lines <- readLines(src_file, warn = FALSE)
  body <- yconv_source_body(
...
test_that("target-active raw and Bernstein supertiles are repeatable", {
  set.seed(20260724)
  n <- 1025L
