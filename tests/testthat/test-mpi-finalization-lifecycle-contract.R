...
test_that("native MPI lifecycle state and finalization are idempotent", {
  ...
  result <- npRmpi_run_rscript_subprocess(
    lines = c(
      ...
      "for (fn in c('mpi_comm_rank','mpi_comm_size','mpi_comm_is_null','mpi_barrier','mpi_comm_free')) {",
      "  e <- tryCatch(.Call(fn, 0L, PACKAGE='npRmpi'), error=identity)",
      "  stopifnot(inherits(e,'error'), grepl('communicator storage is unavailable',conditionMessage(e),fixed=TRUE))",
      "}",
      "e <- tryCatch(.Call('mpi_realloc_comm', 20L, PACKAGE='npRmpi'), error=identity)",
      "stopifnot(inherits(e,'error'), grepl('communicator storage is unavailable',conditionMessage(e),fixed=TRUE))",
      "e <- tryCatch(.Call('mpi_comm_rank', NA_integer_, PACKAGE='npRmpi'), error=identity)",
      "stopifnot(inherits(e,'error'), grepl('must not be NA',conditionMessage(e),fixed=TRUE))",
