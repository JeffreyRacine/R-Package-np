...
npunitest_fun <- function(...) {
  getFromNamespace("npunitest", "npRmpi")(...)
}

test_that("npunitest emits append-only bounded bootstrap progress", {
  skip_on_cran()
  ...
  expect_s3_class(res$value, "unitest")
  expect_true(any(grepl("^\\[npRmpi\\] Computing bandwidths$", messages)))
  expect_true(any(grepl("^\\[npRmpi\\] Computing test statistic$", messages)))
