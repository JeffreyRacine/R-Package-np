test_that("npqreg selected-CDF inversion honors iteration, midpoint, and clamp contracts", {
  inv <- getFromNamespace(".npqreg_invert_selected_cdf", "np")
  validate_itmax <- getFromNamespace(".npqreg_validate_itmax", "np")
  ...
  clamp_cdf <- function(bws, xdat, ydat, exdat, ycand) {
    ifelse(exdat$x == 1, 0.9,
           ifelse(exdat$x == 2, 0.1, (as.double(ycand) + 1) / 2))
  }
  out3 <- inv(bws, xdat, ydat, exdat3, tau = 0.5, tol = 0.5,
              small = .Machine$double.eps, itmax = 10L,
              cdf.values = clamp_cdf)
  expect_equal(as.numeric(out3[1:2]), c(-1, 1), tolerance = 0)
  expect_identical(quantile_clamp(out3), c("lower", "upper", "none"))
  ...
  masked <- mark_clamped_delta(delta, quantile_clamp(out3))
  expect_true(all(is.na(masked$quanterr[1:2])))
  expect_false(is.na(masked$quanterr[3]))
  expect_true(all(is.na(masked$quantgrad[1:2, , drop = FALSE])))
  expect_true(all(is.na(masked$quantgerr[1:2, , drop = FALSE])))
  expect_equal(masked$quantgrad[3, ], c(1, 1))
})

test_that("npqreg direct evaluation preserves NA rows for vector tau gradients", {
  set.seed(20260703)
...
test_that("npqreg direct evaluation follows conditional distribution eval guards", {
  set.seed(20260703)
  xdat <- data.frame(
