...
test_that("npindexhat lp gradient owner preserves fit and matrix/apply parity", {
  set.seed(20260316)
  n <- 90L
  x1 <- runif(n, -1, 1)
  x2 <- rnorm(n)
  ...
  for (cfg in cases) {
    ...
    expect_equal(as.vector(apply.grad), as.vector(fit.grad$grad[, 1L]), tolerance = 1e-8, info = paste("grad", cfg$label))
    expect_equal(as.vector(matrix.grad %*% y), as.vector(fit.grad$grad[, 1L]), tolerance = 1e-8, info = paste("grad matrix", cfg$label))
    expect_equal(as.vector(apply.grad), as.vector(matrix.grad %*% y), tolerance = 1e-10, info = paste("grad parity", cfg$label))
  }
})
