test_that("operator weight policy owns beta and legacy complete units", {
  ...
  for (family in c("gaussian", "beta")) {
    ...
    info <- npRmpi:::.np_operator_kernel_weight_scale(
      bws = bw,
      operator = operators,
      nvars = ncol(dat),
    ...
    explicit <- npRmpi:::.np_kernel_weights_direct(
      bws = info$bws,
      txdat = dat,
      exdat = eval,
    ...
    got <- npRmpi:::.np_ksum_unconditional_operator_fixed(
...
test_that("audited operator consumers use the shared unit policy", {
  ...
  expect_identical(
    lengths(regmatches(
      paste(plot.source, cdhat.source, sep = "\n"),
      gregexpr(".np_operator_kernel_weight_scale <- function",
               paste(plot.source, cdhat.source, sep = "\n"), fixed = TRUE)
    )),
    1L
  )
