test_that("CVML raw invalid cache cannot become finite likelihood guidance", {
  ...
  guided <- get("npRmpiPreparedObjectiveEvalConditionalDensity",ns)
  raw <- get("npRmpiPreparedObjectiveEvalConditionalDensityRaw",ns)
  ...
  command <- substitute(PROBE(),list(PROBE=probe))
  out <- npRmpi:::.npRmpi_bcast_cmd_expr(command,caller.execute=TRUE)
