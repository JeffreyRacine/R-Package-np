test_that("npcdistbw formula and default paths preserve current metadata shape", {
  ...
  expect_s3_class(bw_formula, "condbandwidth")
  expect_s3_class(bw_default, "condbandwidth")
  ...
  expect_identical(as.character(bw_formula$call[[1L]]), "npcdistbw.formula")
  expect_identical(as.character(bw_default$call[[1L]]), "npcdistbw.NULL")
...
test_that("npcdistbw formula path preserves omitted-row metadata", {
  old_opts <- options(np.messages = FALSE, np.tree = FALSE)
  on.exit(options(old_opts), add = TRUE)

  dat <- data.frame(
    y = c(0.1, 0.2, NA, 0.4, 0.5, 0.6),
...
test_that("npcdistbw formula path handles non-syntactic variable names", {
  old_opts <- options(np.messages = FALSE, np.tree = FALSE)
  on.exit(options(old_opts), add = TRUE)
...
test_that("npcdistbw rejects mismatched x bandwidth object types with current message", {
  old_opts <- options(np.messages = FALSE, np.tree = FALSE)
  on.exit(options(old_opts), add = TRUE)
