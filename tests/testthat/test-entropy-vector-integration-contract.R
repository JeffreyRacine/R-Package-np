test_that("deterministic bivariate entropy quadrature preserves its oracle", {
  ...
  domain.helper <- getFromNamespace(".np_entropy_bivariate_domain", "npRmpi")
  set.seed(20260717)
  n <- 24L
  x <- rnorm(n)
...
test_that("entropy trapezoid weights match the established helper", {
  weights <- getFromNamespace(".np_entropy_trapezoid_weights", "npRmpi")
  established <- getFromNamespace("integrate.trapezoidal", "npRmpi")
