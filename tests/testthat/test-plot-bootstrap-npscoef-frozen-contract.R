npscoef_semiparam_frozen_contract_case <- function() {
  ...
  lines <- c(
    ...
    "    as.vector(np::npscoef(bws = bw_np, txdat = xdat[idx, , drop = FALSE], tzdat = zdat[idx, , drop = FALSE], tydat = y[idx], exdat = exdat, ezdat = ezdat, iterate = FALSE, se = FALSE)$mean)",
    "  }, numeric(nrow(exdat)))",
    "  exact.oracle <- t(exact.oracle)",
    "  t0.oracle <- as.vector(np::npscoef(bws = bw_np, txdat = xdat, tzdat = zdat, tydat = y, exdat = exdat, ezdat = ezdat, iterate = FALSE, se = FALSE)$mean)",
    ...
    "  expected.plot <- plot(bw, xdat = xdat, ydat = y, zdat = zdat, neval = 6L, coef = FALSE, output = 'data')",
...
npscoef_fixed_lp_semiparam_contract_case <- function() {
  ...
  lines <- c(
    ...
    "    idx <- np:::.np_counts_to_indices(counts[, j])",
    "    as.vector(np::npscoef(bws = bw_np, txdat = xdat[idx, , drop = FALSE], tzdat = zdat[idx, , drop = FALSE], tydat = y[idx], exdat = eval.grid['x'], ezdat = eval.grid['z'], iterate = FALSE, se = FALSE)$mean)",
