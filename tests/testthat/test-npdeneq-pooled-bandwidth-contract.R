...
test_that("reference adjustment retains compact symmetric kernel choices", {
  ...
  adjust <- getFromNamespace(".npdeneq_reference_bandwidth", "npRmpi")
...
test_that("harmonic reference scaling retains pooled spread and kernel metadata", {
  ...
  adjust <- getFromNamespace(".npdeneq_reference_bandwidth", "npRmpi")
  kbw <- getFromNamespace("kbandwidth", "npRmpi")
...
test_that("automatic selection sees both complete samples exactly once", {
  ...
  # A master-only synthetic selector must not be inserted into a collective
  # public call: workers would perform a real search while the master skipped
  # it. Probe the preparation owner directly; the next test exercises the
  # public collective route with an actual pooled search on every rank.
  support <- getFromNamespace(".npdeneq_common_support", "npRmpi")(
    stats::na.omit(x), stats::na.omit(y))
  common <- getFromNamespace(".npdeneq_common_bandwidth", "npRmpi")(
    support$x, support$y, NULL, NULL)
  actual <- npdeneqtest(x, y, bw.x = common, B = 9)
