...
test_that("npindexbw emits append-only bounded multistart progress", {
  ...
  expect_true(any(grepl("^\\[np\\] Selecting single-index bandwidth 1/3 \\([0-9]+\\.[0-9]%.*, elapsed [0-9]+\\.[0-9]s, eta [0-9]+\\.[0-9]s\\): multistart 1 of 3$", messages)))
  expect_true(any(grepl("^\\[np\\] Selecting single-index bandwidth 3/3 \\([0-9]+\\.[0-9]%.*, elapsed [0-9]+\\.[0-9]s, eta [0-9]+\\.[0-9]s\\): multistart 3 of 3$", messages)))
