test_that("ragged panel ranges accept only declared trailing NA padding", {
  all.range <- getFromNamespace("compute.all.error.range", "np")
  default.range <- getFromNamespace("compute.default.error.range", "np")
  for (n in 1:5) {
    centre <- seq_len(n) / 7
...
test_that("all-band rendering uses the same unpadded interval rows as range calculation", {
  draw <- getFromNamespace("draw.all.error.types", "np")
  rows <- list()
