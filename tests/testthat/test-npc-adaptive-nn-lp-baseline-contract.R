...
make_adaptive_nn_conditional_data <- function() {
  ...
  ex <- seq(0.1, 0.9, length.out = 40L)
  list(
    txdat = data.frame(x = x),
    tydat = data.frame(y = y),
    exdat = data.frame(x = ex),
    eydat = data.frame(y = sin(5 * ex) + 0.3 * ex^2)
  )
}
...
test_that("failed local higher-order regression cannot poison MPI conditional gradients", {
  if (!spawn_mpi_slaves(1L)) skip("Could not spawn MPI slaves")
  on.exit(close_mpi_slaves(force = TRUE), add = TRUE)
  ...
  expect_error(
    local_regression(npreg(
      bws = bw, exdat = x, gradients = TRUE, gradient.order = 2L
    )),
