...
test_that("npdeneqtest single-line bootstrap progress matches legacy semantics", {
  skip_on_cran()
  ...
  set.seed(42)
  x <- data.frame(v = rnorm(30))
  y <- data.frame(v = rnorm(30))

  old_opts <- options(np.messages = TRUE)
  on.exit(options(old_opts), add = TRUE)

  legacy <- capture_progress_shadow_trace(
    npdeneqtest_fun(x = x, y = y, boot.num = 9),
  ...
  expect_s3_class(single_line$value, "deneqtest")
...
test_that("npdeneqtest progress respects suppressMessages", {
  ...
  res <- capture_progress_shadow_trace(
    suppressMessages(npdeneqtest_fun(x = x, y = y, boot.num = 9)),
    now = progress_time_counter()
  )
