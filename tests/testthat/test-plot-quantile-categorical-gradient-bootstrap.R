test_that("session-route quantile categorical gradient helper matches explicit local refits", {
  ...
  res <- npRmpi_run_rscript_subprocess(
    lines = c(
      ...
      "tau <- 0.5",
      "slice <- suppressWarnings(plot(bw, xdat = xdat, ydat = ydat, tau = tau, gradients = TRUE, perspective = FALSE, output = 'data'))[[1L]]",
...
test_that("session-route quantile categorical bootstrap gradients work again", {
  ...
  res <- npRmpi_run_rscript_subprocess(
    lines = c(
      ...
      "    plot.args <- c(list(case$obj, tau = 0.5, gradients = TRUE, perspective = FALSE, output = 'data', errors = 'bootstrap', bootstrap = boot.method, B = 5L), case$args)",
