...
test_that("npRmpi semiparametric formula fits preserve response-name metadata", {
  ...
  res <- npRmpi_run_rscript_subprocess(
    lines = c(
      ...
      "fit_sc <- npscoef(y ~ x | z, data = dat_sc, nmulti = 1, regtype = 'lp', degree = 1, bwtype = 'adaptive_nn', se = FALSE, betas = FALSE)",
      ...
      "fit_sc_formula <- npscoef(y ~ x1 + x2 | z1 + z2 + z3, data = dat_fn, bws = rep(0.2, 3L), se = FALSE, betas = FALSE)",
      "bw_sc_object <- npscoefbw(xdat = xdat_fn, ydat = dat_fn$y, zdat = zdat_fn, bws = rep(0.2, 3L), bandwidth.compute = FALSE)",
      "fit_sc_object <- npscoef(bws = bw_sc_object, txdat = xdat_fn, tydat = dat_fn$y, tzdat = zdat_fn, se = FALSE, betas = FALSE)",
