npplreg_semiparam_frozen_contract_case <- function() {
  ...
  lines <- c(
    ...
    "  bootstrap.plot <- NULL",
    "  invisible(capture.output(bootstrap.plot <- plot(bw, xdat = txdat, ydat = y, zdat = tzdat, neval = 3L, output = 'data', errors = 'bootstrap', bootstrap = 'inid', boot.control = np_boot_control(nonfixed = 'frozen'), B = 41L, band = 'pointwise')))",
