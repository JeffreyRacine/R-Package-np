test_that("session-route conditional categorical gradient helper matches explicit local refits", {
  ...
  res <- npRmpi_run_rscript_subprocess(
    lines = c(
      ...
      "  slice <- suppressWarnings(plot(fixture$bw, xdat = xdat, ydat = ydat, gradients = TRUE, perspective = FALSE, behavior = 'data'))[[1L]]",
...
test_that("session-route conditional categorical bootstrap gradients work again", {
  ...
  res <- npRmpi_run_rscript_subprocess(
    lines = c(
      ...
      "    plot.args <- c(list(case$obj, perspective = FALSE, behavior = 'data', gradients = TRUE, errors = 'bootstrap', bootstrap = boot.method, B = 7L), case$args)",
