...
test_that("tree and categorical compression predicates are orthogonal", {
  ...
  observed <- lapply(seq_len(nrow(cases)), function(i) {
    ...
    c(
      categorical = np:::npUseCategoricalCompress(ncon = 0L, ncat = 2L),
      continuous_tree = np:::npUseContinuousTree(ncon = 2L),
      mixed_flag = np:::npUseKernelAccelerationFlag(ncon = 1L, ncat = 2L),
      allcat_flag = np:::npUseKernelAccelerationFlag(ncon = 0L, ncat = 2L)
    )
  })
...
test_that("np.tree auto mode is bounded-kernel continuous tree only", {
  ...
  expect_true(np:::npUseContinuousTree(ncon = 1L, ckertype = "uniform"))
  ...
  expect_false(np:::npUseCategoricalCompress(ncon = 0L, ncat = 2L))
  options(np.categorical.compress = TRUE)
  expect_true(np:::npUseCategoricalCompress(ncon = 0L, ncat = 2L))

  options(np.tree = TRUE)
  expect_true(np:::npUseContinuousTree(ncon = 1L, ckertype = "gaussian"))

  options(np.tree = FALSE)
  expect_false(np:::npUseContinuousTree(ncon = 1L, ckertype = "epanechnikov"))
})
...
test_that("np.tree alone does not enable all-categorical profile helpers", {
  ...
  expect_null(np:::.np_regression_cat_profile_mean(bws, xdat, ydat))

  options(np.tree = FALSE, np.categorical.compress = TRUE)
  profile.mean <- np:::.np_regression_cat_profile_mean(bws, xdat, ydat)
  expect_type(profile.mean, "double")
  expect_length(profile.mean, n)

  options(np.tree = TRUE, np.categorical.compress = FALSE)
  expect_equal(
    np:::.npreg_fit_tree_code(bws, ncon = bws$ncon, ncat = bws$nuno + bws$nord),
    np:::DO_TREE_NO
  )
...
test_that("np.tree auto inspects active bandwidth-object kernels", {
  ...
  expect_equal(
    np:::.npreg_fit_tree_code(bw.epan, ncon = bw.epan$ncon, ncat = 0L),
    np:::DO_TREE_YES
  )
  expect_equal(
    np:::.npreg_fit_tree_code(bw.gauss, ncon = bw.gauss$ncon, ncat = 0L),
    np:::DO_TREE_NO
  )
})
...
test_that("continuous and mixed tree route helpers still honor np.tree", {
  ...
  expect_true(np:::npUseContinuousTree(ncon = bws$ncon))
  expect_false(np:::npUseCategoricalCompress(ncon = bws$ncon,
