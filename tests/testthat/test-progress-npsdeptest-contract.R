...
test_that("npsdeptest emits append-only bounded lag and bootstrap progress", {
  ...
  expect_true(any(grepl("^\\[npRmpi\\] Constructing metric entropy by lag 1/2 \\([0-9]+\\.[0-9]%.*, elapsed [0-9]+\\.[0-9]s, eta [0-9]+\\.[0-9]s\\): lag 1$", messages)))
  expect_true(any(grepl("^\\[npRmpi\\] Constructing metric entropy by lag 2/2 \\([0-9]+\\.[0-9]%.*, elapsed [0-9]+\\.[0-9]s, eta [0-9]+\\.[0-9]s\\): lag 2$", messages)))
