...
test_that("npsdeptest single-line lag and bootstrap progress match legacy semantics", {
  ...
  expect_true(any(grepl("^\\[np\\] Computing entropy statistics by lag 1/2 \\([0-9]+\\.[0-9]%.*, elapsed [0-9]+\\.[0-9]s, eta [0-9]+\\.[0-9]s\\)$", lines)))
  expect_true(any(grepl("^\\[np\\] Computing entropy statistics by lag 2/2 \\([0-9]+\\.[0-9]%.*, elapsed [0-9]+\\.[0-9]s, eta [0-9]+\\.[0-9]s\\)$", lines)))
