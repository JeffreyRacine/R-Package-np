test_that("plot bootstrap display owns one clock without changing local clocks", {
  ...
  trace <- capture_progress_shadow_trace({
    result <- np:::.np_plot_progress_run({
      np:::.np_plot_progress_plan(2L)
      target <- np:::.np_plot_progress_target_begin()
      now <- 2
      local <- np:::.np_plot_bootstrap_progress_begin(10L, "Plot bootstrap")
      expect_equal(local$started, 2)
      throttle <- local$throttle_sec
      now <- 5
      local <- np:::.np_plot_progress_tick(local, 5L, force = TRUE)
      expect_equal(local$started, 2)
      expect_identical(local$throttle_sec, throttle)
      np:::.np_plot_progress_end(local)
      np:::.np_plot_progress_target_end(target)
      target <- np:::.np_plot_progress_target_begin()
      now <- 8
      local <- np:::.np_plot_bootstrap_progress_begin(10L, "Plot bootstrap")
      expect_equal(local$started, 8)
      np:::.np_fit_progress_step(1L)
      now <- 12
      local <- np:::.np_plot_progress_tick(local, 10L, force = TRUE)
      np:::.np_plot_progress_end(local)
      np:::.np_plot_progress_target_end(target)
      17L
    }, enabled = TRUE)
  ...
  expect_null(np:::.np_plot_progress_runtime$context)
  expect_null(np:::.np_progress_runtime$fit_forward)
})
...
test_that("nested and failed plots restore progress scope without false completion", {
  ...
  result <- capture_progress_shadow_trace({
    expect_error(np:::.np_plot_progress_run({
      np:::.np_plot_progress_plan(3L)
      outer <- np:::.np_plot_progress_runtime$context
      np:::.np_plot_progress_run({
        np:::.np_plot_progress_plan(99L)
        expect_null(np:::.np_plot_progress_runtime$context)
      }, enabled = TRUE)
      expect_identical(np:::.np_plot_progress_runtime$context, outer)
    ...
    expect_null(np:::.np_plot_progress_runtime$context)
    expect_null(np:::.np_progress_runtime$fit_forward)
    expect_null(np:::.np_progress_registry$active_id)
  }, force_renderer = "single_line", now = function() now)
...
test_that("display aggregation leaves chunk-controller decisions unchanged", {
  ...
  capture_progress_shadow_trace({
    for (aggregate in c(FALSE, TRUE)) {
      sizes[[length(sizes) + 1L]] <- np:::.np_plot_progress_run({
        progress <- np:::.np_plot_bootstrap_progress_begin(99L, "chunks")
        controller <- np:::.np_plot_progress_chunk_controller(16L, progress)
        inputs <- controller
        chunks <- integer()
        for (elapsed in c(.1, .4, 2, 4, .8)) {
          chunks <- c(chunks, controller$chunk.size)
          controller <- np:::.np_plot_progress_chunk_observe(
            controller, controller$chunk.size, elapsed)
        }
        np:::.np_plot_progress_end(progress)
...
test_that("quiet and standalone bootstrap helpers retain their local ownership", {
  ...
  trace <- capture_progress_shadow_trace({
    np:::.np_plot_progress_run({
      expect_null(np:::.np_plot_progress_runtime$context)
      local <- np:::.np_plot_bootstrap_progress_begin(3L, "standalone")
      expect_null(local$plot_context)
      np:::.np_plot_progress_end(local)
    }, enabled = FALSE)
  }, force_renderer = "single_line")
  expect_true(any(vapply(trace$trace, function(x) grepl("standalone", x$line), TRUE)))
  quiet <- capture_progress_shadow_trace(np:::.np_plot_progress_run({
    expect_null(np:::.np_plot_progress_runtime$context)
    1L
...
test_that("plot display scope preserves visibility and narrow-width work fields", {
  ...
  now <- 0
  trace <- capture_progress_shadow_trace({
    value <- withVisible(np:::.np_plot_progress_run({
      np:::.np_plot_progress_plan(42L)
      target <- np:::.np_plot_progress_target_begin()
      now <- 12
      np:::.np_plot_progress_notify(done = 50L, total = 99L, force = TRUE)
      np:::.np_plot_progress_target_end(target)
      invisible(7L)
    }, enabled = TRUE))
...
test_that("verbose target labels cannot displace progress counters at narrow widths", {
  ...
  for (width in c(60L, 80L, 120L)) {
    ...
    result <- with_np_progress_bindings(
      ...
      capture_progress_shadow_trace(np:::.np_plot_progress_run({
        np:::.np_plot_progress_plan(42L)
        np:::.np_plot_progress_target_begin()
        now <- 12
        np:::.np_plot_progress_notify(
