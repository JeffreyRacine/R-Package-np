...
test_that("fixed sparse width six has Apple-arm64 pair and owned-row siblings", {
  src_dir <- file.path(testthat::test_path("..", ".."), "src")
  row_header <- file.path(src_dir, "jksum_lp_row.h")
  sum_file <- file.path(src_dir, "jksum.c")
  ...
  expect_equal(
    lengths(regmatches(
      sum_text,
      gregexpr("np_lp_accumulate_sparse_row_resident6\\(", sum_text)
    )),
    2L
  )
  expect_match(sum_text, "np_lp_accumulate_sparse_pair_wide_resident\\(")
  expect_match(sum_text, "np_lp_accumulate_sparse_row_wide_resident\\(")
})

test_that("MPI owned rows specialize widths two through six", {
  src_dir <- file.path(testthat::test_path("..", ".."), "src")
  row_file <- file.path(src_dir, "jksum_lp_row.c")
  sum_file <- file.path(src_dir, "jksum.c")
  skip_if_not(file.exists(row_file) && file.exists(sum_file),
