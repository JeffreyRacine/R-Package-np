  ...
  for (kernel in c("gaussian", "beta")) {
    for (bwtype in c("fixed", "generalized_nn", "adaptive_nn")) {
      ...
      for (operator.name in c("normal", "integral")) {
        ...
        got <- np:::.np_exact_operator_apply(
          kbw = bw,
          txdat = train,
          exdat = eval,
          operator = operator,
        ...
        if (identical(bwtype, "fixed") || identical(kernel, "beta")) {
          direct <- np:::.np_direct_operator_apply(
            kbw = bw,
            txdat = train,
            exdat = eval,
            operator = operator,
...
test_that("native output extents fail before integer overflow", {
  expect_identical(
    np:::.np_native_output_extent(7L, 3L, where = "test extent"),
    21L
  )
  expect_error(
    np:::.np_native_output_extent(.Machine$integer.max, 2L,
                                  where = "test extent"),
