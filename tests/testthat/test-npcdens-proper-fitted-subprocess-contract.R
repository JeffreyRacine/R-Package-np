...
test_that("npcdens formula route keeps estimator-only proper arguments out of bandwidth selection", {
  skip_on_cran()
  skip_slow_npcdens_fitted_subprocess()
  ...
  res <- npRmpi_run_rscript_subprocess(
    lines = c(
      ...
      "x <- rnorm(40)",
      "y <- x + rnorm(40)",
      "fit <- npcdens(y ~ x, regtype='lp', degree=1L, bernstein.basis=TRUE, proper=TRUE, gradients=TRUE, nmulti=1)",
      ...
      "stopifnot(all(is.finite(fit$condens)))",
      "stopifnot(all(is.finite(fit$congrad)))",
      "cat('NPCDENS_FORMULA_PROPER_ARGS_OK\\n')"
    ),
    timeout = 120L,
    env = env
  )
  ...
  expect_true(any(grepl("NPCDENS_FORMULA_PROPER_ARGS_OK", res$output, fixed = TRUE)),
