...
beta_cvml_ratio_oracle <- function(x, y, bandwidth, type,
  ...
  if (!identical(type,"fixed")) {
    xweights <- workhorse_nn_loo_weights(x,bandwidth[2L],type,xkernel,order)
    yweights <- workhorse_nn_loo_weights(y,bandwidth[1L],type,ykernel,order)
    fit <- colSums(xweights*yweights)/colSums(xweights)
    return(-sum(beta_cvml_guard(fit)))
  }
