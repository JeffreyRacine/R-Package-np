test_that("regression and index public metadata agree with canonical engine state", {
  validate <- getFromNamespace("npValidatedConditionalRegSpec", "np")
...
test_that("regression boundaries reject missing or incoherent engine state", {
...
test_that("single-index consumers reject inferred or mutated engine state", {
  old <- options(np.messages = FALSE, np.tree = FALSE)
  on.exit(options(old), add = TRUE)
  ...
  n <- 32L
  x <- data.frame(x1 = runif(n), x2 = runif(n))
  y <- sin(x$x1 + x$x2) + rnorm(n, sd = 0.1)
  bw <- npindexbw(
    xdat = x,
    ydat = y,
    bws = c(1, 1, 0.3),
  ...
  missing.state <- bw
  names(missing.state)[names(missing.state) == "basis.engine"] <-
    "basis.engine.shadow"
  expect_error(
    npindex(bws = missing.state, txdat = x, tydat = y),
    "npindex is missing required field 'basis.engine'",
    fixed = TRUE
  )

  incoherent <- bw
  incoherent$degree.engine <- 1L
  expect_error(
    npindex(bws = incoherent, txdat = x, tydat = y),
