test_that("npindex internal geometry consumes the canonical numeric matrix", {
  validate <- getFromNamespace(".npindex_internal_numeric_matrix", "np")
  index_from_beta <- getFromNamespace(".npindex_index_from_beta_tail", "np")
  coordinate_setup <- getFromNamespace(".npindex_beta_coordinate_setup", "np")
