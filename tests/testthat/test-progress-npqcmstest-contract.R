...
npqcmstest_fun <- function(...) {
  getFromNamespace("npqcmstest", "npRmpi")(...)
}
...
test_that("npqcmstest single-line bootstrap progress matches legacy semantics", {
  ...
  legacy <- capture_progress_shadow_trace(
    npqcmstest_fun(model = model, xdat = x, ydat = y, distribution = "bootstrap", boot.num = 9),
...
test_that("npqcmstest progress respects suppressMessages", {
  ...
  res <- capture_progress_shadow_trace(
    suppressMessages(npqcmstest_fun(model = model, xdat = x, ydat = y, distribution = "bootstrap", boot.num = 9)),
    now = progress_time_counter()
  )
...
test_that("npqcmstest source routes use canonical bootstrap surface tags", {
  src <- paste(readLines(testthat::test_path("..", "..", "R", "np.qcmstest.R"), warn = FALSE), collapse = "\n")
