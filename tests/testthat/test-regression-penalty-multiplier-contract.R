...
test_that("regression penalty multipliers have a strict public boundary", {
  validate <- getFromNamespace(
    "npValidateRegressionPenaltyMultiplier", "npRmpi"
  )
...
test_that("internal and native regression ingresses reject subunit multipliers", {
  ...
  prep <- prepare(x, y, bw)
  native.eval <- function(multiplier) local.regression(.Call(
  ...
  expect_true(is.finite(native.eval(prep$penalty_multiplier)$fval[[1L]]))
})
