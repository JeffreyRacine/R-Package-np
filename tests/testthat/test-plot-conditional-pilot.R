cf75_get <- function(name) getFromNamespace(name, "np")
cf75_local <- function(expr) {
  if ("np" == "npRmpi") cf75_get(".npRmpi_with_local_regression")(force(expr))
  else force(expr)
}
    ...
    for(method in c("fixed","geom")) {
      ...
      out <- withCallingHandlers({
        ...
        if(identical(conditionMessage(w),
          "[np] ignoring kernel order specified with uniform kernel type")) {
