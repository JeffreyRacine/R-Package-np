...
test_that("fixed and generalized-NN CVLS siblings preserve consumer order", {
...
test_that("CVLS row reuse covers fixed and generalized NN but not npcdist", {
  src_file <- locate_cvls_row_reuse_source()
  skip_if(is.null(src_file), "source file src/jksum.c unavailable in this test context")
  lines <- readLines(src_file, warn = FALSE)
  ...
  expect_true(fixed_start > generalized_start)
  generalized_block <- substr(density_body, generalized_start, fixed_start - 1L)
  expect_true(grepl(
  ...
  shared_y_body <- cvls_source_body(
    ...
    "^static int np_conditional_x_block_bw_ctx_prepare\\("
  )
  expect_match(
    shared_y_body,
...
test_that("fixed CVLS row reuse preserves repeatable multivariate degree objectives", {
  ...
  first <- np:::.npcdensbw_eval_only(x, y, bw)
  second <- np:::.npcdensbw_eval_only(x, y, bw)
