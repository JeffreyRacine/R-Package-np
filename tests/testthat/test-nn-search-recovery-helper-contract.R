test_that("ordinary NN recovery schedules only declared coordinates", {
  ...
  schedule <- npRmpi:::.np_nn_ordinary_schedule(
    point = point,
    nn.indices = c(1L, 3L),
    caps = c(9L, 6L)
  )
...
test_that("ordinary NN recovery uses raw validity and one deterministic order", {
  ...
  raw.eval <- function(point) {
    ...
    if (point[[1L]] < 8) {
      npRmpi:::.np_nn_abort_candidate_invalid(
  ...
  result <- npRmpi:::.np_nn_find_raw_valid_start(
    point = c(2, 0.3, 2, 0.7),
    nn.indices = c(1L, 3L),
    caps = c(9L, 7L),
    raw.eval = raw.eval
...
test_that("ordinary NN recovery propagates unknown errors", {
  expect_error(
    npRmpi:::.np_nn_find_raw_valid_start(
      point = c(2, 2),
      nn.indices = 1:2,
      caps = c(8, 8),
...
test_that("raw objective certification rejects penalties at the terminal scale", {
  expect_true(npRmpi:::.np_nn_raw_objective_valid(0))
  expect_true(npRmpi:::.np_nn_raw_objective_valid(-1e7))
  expect_false(npRmpi:::.np_nn_raw_objective_valid(NA_real_))
  expect_false(npRmpi:::.np_nn_raw_objective_valid(Inf))
  expect_false(npRmpi:::.np_nn_raw_objective_valid(.Machine$double.xmax))
  expect_false(npRmpi:::.np_nn_raw_objective_valid(-.Machine$double.xmax))
})
...
test_that("raw endpoint certification returns a scalar or a typed condition", {
  point <- c(4, 0.25)
  expect_identical(
    npRmpi:::.np_nn_certify_raw_point(
