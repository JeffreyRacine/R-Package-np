...
test_that("npscoef width-one and width-two systems share the native batch solve", {
  for (p in 1:2) {
    ...
    actual <- np:::.npscoef_batch_zero_solve(tyw = tyw, tww = tww)
    expect_equal(actual, expected, tolerance = 1e-13)

    singular <- tww
    singular[, , 3L] <- 0.0
    expect_null(np:::.npscoef_batch_zero_solve(tyw = tyw, tww = singular))

    nonfinite <- tww
    nonfinite[1L, 1L, 3L] <- NaN
    expect_null(np:::.npscoef_batch_zero_solve(tyw = tyw, tww = nonfinite))

    if (p == 2L) {
      ill_conditioned <- tww
      ill_conditioned[, , 3L] <- diag(c(1.0, .Machine$double.eps^2))
      expect_null(np:::.npscoef_batch_zero_solve(
        tyw = tyw,
        tww = ill_conditioned
      ))
    }
...
test_that("npscoef batch projection matches incumbent row crossproducts numerically", {
  ...
  for (nbasis in c(1L, 3L, 6L, 9L, 15L, 26L)) {
    ...
    actual <- np:::.npscoef_batch_project(theta, Wz.eval)
