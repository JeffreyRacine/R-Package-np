...
# These are local numerical npRmpi contracts, not MPI fanout contracts.  Keep
# autodispatch disabled and do not spawn a worker pool for every test block.
npRmpi_local_regression <- function(expr) {
  local_regression <- get(".npRmpi_with_local_regression",
                          envir = asNamespace("npRmpi"))
  expr <- substitute(expr)
  eval(substitute(LOCAL(EXPR), list(LOCAL = local_regression, EXPR = expr)),
       envir = parent.frame())
}
...
test_that("npreg lp higher-order gradients match npreghat across lp bases", {
  ...
  tx <- data.frame(x = x)
  ex <- data.frame(x = seq(min(x), max(x), length.out = 3))

  for (basis in c("glp", "additive", "tensor")) {
    bw <- npRmpi_local_regression(npregbw(
      xdat = tx,
      ydat = y,
      regtype = "lp",
      degree = 2L,
    ...
    fit <- npRmpi_local_regression(suppressWarnings(npreg(
      txdat = tx,
      tydat = y,
      exdat = ex,
      bws = bw,
    ...
    H2 <- npRmpi_local_regression(npreghat(bws = bw, txdat = tx,
                                           exdat = ex, s = 2L))
    g2 <- as.vector(H2 %*% y)
...
test_that("npreg lp supports per-variable derivative orders", {
  ...
  ex <- data.frame(x1 = seq(0.05, 0.95, length.out = 3),
                   x2 = seq(0.1, 0.9, length.out = 3))
  ...
  Hx1 <- npRmpi_local_regression(npreghat(bws = bw, txdat = tx,
                                          exdat = ex, s = c(2L, 0L)))
  Hx2 <- npRmpi_local_regression(npreghat(bws = bw, txdat = tx,
                                          exdat = ex, s = c(0L, 1L)))
...
test_that("npreg lp mixed-degree derivatives preserve partial availability", {
  ...
  tx <- data.frame(x1 = x1, x2 = x2)
  ex <- data.frame(x1 = seq(0.1, 0.9, length.out = 3),
                   x2 = seq(0.15, 0.85, length.out = 3))
  ...
  expect_warning(
    fit1 <- npRmpi_local_regression(npreg(
      txdat = tx,
      tydat = y,
      exdat = ex,
      bws = bw,
  ...
  H01 <- npRmpi_local_regression(npreghat(bws = bw, txdat = tx,
                                          exdat = ex, s = c(0L, 1L)))
  ...
  expect_error(
    npRmpi_local_regression(npreghat(bws = bw, txdat = tx, exdat = ex,
                                     s = c(1L, 0L))),
    "exceeds local polynomial degree"
  )

  expect_warning(
    fit2 <- npRmpi_local_regression(npreg(
      txdat = tx,
      tydat = y,
      exdat = ex,
      bws = bw,
  ...
  H02 <- npRmpi_local_regression(npreghat(bws = bw, txdat = tx,
                                          exdat = ex, s = c(0L, 2L)))
  ...
  bw00 <- npRmpi_local_regression(npregbw(
    xdat = tx,
    ydat = y,
    regtype = "lp",
    degree = c(0L, 0L),
  ...
  fit00 <- npRmpi_local_regression(npreg(
    txdat = tx,
    tydat = y,
    exdat = ex,
    bws = bw00,
  ...
  H00.10 <- npRmpi_local_regression(npreghat(bws = bw00, txdat = tx,
                                             exdat = ex, s = c(1L, 0L)))
  H00.01 <- npRmpi_local_regression(npreghat(bws = bw00, txdat = tx,
                                             exdat = ex, s = c(0L, 1L)))
  ...
  bw01 <- npRmpi_local_regression(npregbw(
    xdat = tx,
    ydat = y,
    regtype = "lp",
    degree = c(0L, 1L),
  ...
  expect_error(
    npRmpi_local_regression(npreg(txdat = tx, tydat = y, exdat = ex,
                                  bws = bw01, gradients = TRUE,
                                  gradient.order = 2L)),
  ...
  expect_warning(
    plot.out <- npRmpi_local_regression(plot(
      bw, xdat = tx, ydat = y, gradients = TRUE, gradient.order = 2L,
      errors = "none", output = "data", neval = 8L
    )),
    "exceed polynomial degree"
  )
...
test_that("npreg lp gradients accessor honors stored derivative order", {
  ...
  bw2 <- npRmpi_local_regression(npregbw(
    xdat = tx,
    ydat = y,
    regtype = "lp",
    degree = 2L,
  ...
  fit.degree1 <- npRmpi_local_regression(npreg(
    txdat = tx,
    tydat = y,
    bws = bw1,
    gradients = TRUE,
...
test_that("npreg lp Bernstein derivatives are returned on original scale", {
  ...
  fit <- npRmpi_local_regression(npreg(
    txdat = tx,
    tydat = y,
    bws = bw,
    gradients = TRUE,
