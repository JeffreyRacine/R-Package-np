test_that("nplsqreg direct routes omit incomplete training rows and realign output", {
  set.seed(20260703)
  n <- 24L
  dat <- data.frame(
...
test_that("nplsqreg formula routes preserve training and evaluation row shape", {
  set.seed(20260703)
  n <- 24L
  dat <- data.frame(
    x = seq(0.05, 0.95, length.out = n),
...
test_that("nplsqreg exact bandwidth reuse rejects mismatched explicit training data", {
  set.seed(20260703)
  n <- 24L
  dat <- data.frame(
    x = seq(0.05, 0.95, length.out = n),
...
test_that("nplsqreg fixed delta layout is preserved by the check-loss backend", {
  set.seed(20260703)
  n <- 24L
  dat <- data.frame(
    x = seq(0.05, 0.95, length.out = n),
