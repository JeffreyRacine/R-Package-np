...
test_that("npRmpi fixed conditional plot bootstrap covers fixed and geom in subprocess", {
  ...
  res <- npRmpi_run_rscript_subprocess(
    lines = c(
      ...
      "  out.dens <- plot(fit.dens, errors='bootstrap', bootstrap=boot.method, B=9L, boot_control=np_boot_control(blocklen=3L), output = 'data')",
      "  out.dist <- plot(fit.dist, errors='bootstrap', bootstrap=boot.method, B=9L, boot_control=np_boot_control(blocklen=3L), output = 'data')",
