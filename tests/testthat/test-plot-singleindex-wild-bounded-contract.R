test_that("single-index fixed wild bootstrap training mean preserves hat apply", {
  ...
  old.apply <- getFromNamespace(".np_plot_singleindex_hat_apply_index", "npRmpi")
  train.mean <- getFromNamespace(".np_plot_singleindex_training_mean_index", "npRmpi")
...
test_that("single-index fixed wild bootstrap forced block path is finite", {
  if (!spawn_mpi_slaves(n = 2L))
    skip("Could not spawn MPI slaves")
  on.exit(close_mpi_slaves(), add = TRUE)
  ...
  expect_equal(dim(out$merr), c(23L, 2L))
