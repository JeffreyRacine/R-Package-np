test_that("npplreg fixed no-error plot-data prototype matches current route", {
  ...
  y <- 1 + x$x + sin(z$z) + rnorm(n, sd = 0.15)
  cases <- list(
    lc = list(regtype = "lc"),
    ll = list(regtype = "ll"),
  ...
  for (case.name in names(cases)) {
    bw.args <- c(
      list(
        ...
        zdat = z,
        ydat = y,
        bws = matrix(0.45, nrow = 2L, ncol = 1L),
...
test_that("npplreg fixed asymptotic plot-data prototype matches current route", {
  ...
  bw <- npplregbw(
    ...
    zdat = z,
    ydat = y,
    bws = matrix(0.55, nrow = 2L, ncol = 1L),
    bandwidth.compute = FALSE,
    regtype = "ll",
    bwtype = "fixed"
  )
  ...
  for (band in c("pmzsd", "pointwise", "bonferroni", "simultaneous", "all")) {
    ...
    expect_equal(candidate$r1$evalx, old$r1$evalx, info = band)
...
test_that("npplreg fixed bootstrap plot-data prototype matches current route", {
  ...
  for (ii in seq_len(nrow(cases))) {
    ...
    expect_equal(candidate$r1$evalx, old$r1$evalx, info = paste(method, center))
