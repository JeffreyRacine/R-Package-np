test_that("fixed-LP bootstrap batch projection uses one canonical response solve", {
  project <- getFromNamespace(".np_inid_lp_batch_project", "np")
  set.seed(81701)
...
test_that("fixed-LP bootstrap batch projection has typed transactional failures", {
  project <- getFromNamespace(".np_inid_lp_batch_project", "np")
  registered <- getNativeSymbolInfo("C_np_lp_batch_project", PACKAGE = "np")
  expect_identical(registered$dll[["name"]], "np")
  ...
  expect_identical(exhausted$status, 3L)
  expect_null(exhausted$values)
  expect_identical(exhausted$failed_system, 1L)
  expect_identical(exhausted$ridge_steps, 128L)
...
test_that("migrated fixed-LP index and smooth-coefficient consumers match refits", {
  ...
  index.fast <- getFromNamespace(".np_inid_boot_from_index", "np")(
    xdat = index.x,
    ydat = index.y,
    bws = index.bw,
    B = ncol(counts),
  ...
  scoef.fast <- getFromNamespace(".np_inid_boot_from_scoef", "np")(
    txdat = sx,
    ydat = sy,
    tzdat = sz,
    exdat = sex,
...
test_that("migrated fixed-LP partial and conditional consumers match refits", {
  ...
  pl.fast <- getFromNamespace(".np_inid_boot_from_plreg", "np")(
    txdat = px,
    ydat = py,
    tzdat = pz,
    exdat = px,
  ...
  cond.fast <- getFromNamespace(".np_inid_boot_from_ksum_conditional", "np")(
    xdat = cx,
    ydat = cy,
    exdat = cex,
    eydat = cey,
