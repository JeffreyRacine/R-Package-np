...
test_that("joint reuse preserves pure reducer ordering, zeros and diagnostics", {
  ...
  state$calls <- integer()
  state$fits <- list()
  sandbox$.npreg_complete <- function(bws, txdat, tydat, ...) {
...
test_that("actual joint tiles fit once per response and preserve exact component order", {
  ...
  tile <- getFromNamespace(".np_npsig_streamed_iid_tile", "np")
  owner <- getFromNamespace(".npreg_complete", "np")
  ...
  local_mocked_bindings(.npreg_complete = wrapped, .package = "np")
...
test_that("public joint reuse retains bootstrap arrays, metadata and RNG", {
  old <- options(np.messages = FALSE)
  on.exit(options(old), add = TRUE)
  set.seed(727)
  n <- 36L
    ...
    for (pivot in c(FALSE, TRUE)) {
      for (index in list(1:3, c(2L, 3L, 1L))) {
        ...
        expected <- with_mocked_bindings(
          npsigtest(bw, xdat = x, ydat = y, index = index, joint = TRUE,
            B = 9L, pivot = pivot, boot.method = method, random.seed = 727),
          .np_npsig_streamed_response_statistic = j1_uncached, .package = "np")
        after <- .Random.seed
