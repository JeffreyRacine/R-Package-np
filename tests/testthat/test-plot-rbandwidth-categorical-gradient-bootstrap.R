test_that("session-route wild categorical regression gradient helper matches explicit refits", {
  ...
  res <- npRmpi_run_rscript_subprocess(
    lines = c(
      ...
      "exdat <- plot(bw, xdat = xdat, ydat = y, gradients = TRUE, behavior = 'data', perspective = FALSE)[[1L]]$eval",
...
test_that("session-route categorical regression gradient bootstrap works for default and wild routes", {
  ...
  res <- npRmpi_run_rscript_subprocess(
    lines = c(
      ...
      "  args <- list(fit, xdat = xdat, ydat = y, behavior = 'data', perspective = FALSE, gradients = TRUE, errors = 'bootstrap', B = 11L)",
