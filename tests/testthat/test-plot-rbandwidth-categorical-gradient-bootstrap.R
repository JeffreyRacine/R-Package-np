...
test_that("session-route categorical gradient asymptotic intervals use native HC0 errors", {
  ...
  res <- npRmpi_run_rscript_subprocess(
    lines = c(
      ...
      "expected.gerr <- cbind(-qnorm(0.975) * oracle.cat$gerr[, 1L], qnorm(0.975) * oracle.cat$gerr[, 1L])",
      "stopifnot(isTRUE(all.equal(as.vector(out.cat[[1L]]$grad), as.vector(oracle.cat$grad[, 1L]), tolerance = 2e-12)))",
      "stopifnot(isTRUE(all.equal(as.vector(out.cat[[1L]]$gerr), as.vector(expected.gerr), tolerance = 2e-12)))",
