...
* MPI apply and bootstrap collectors now limit the terminal handshake after
  ...
  returning partial results or automatically aborting MPI.
...
* Significance-test preparation uses local regression when automatic dispatch
  is disabled outside a collective broadcast, avoiding an unmatched MPI
  collective. Ordinary automatic and explicit collective preparation retain
  their MPI work sharing.
...
* Significance tests keep their progress heartbeat active during long native
  ...
  Type-I preparation now uses the existing distributed regression computation,
  reducing large-sample preparation time at the cost of additional coordination
  for short jobs. Independently assigned bootstrap fits remain rank-local.

* Density-equality tests display activity while computing the observed statistic
  after the bootstrap replications finish. Eligible observed kernel sums now
  share work across ranks instead of repeating the calculation on every rank.
...
* Single-index formula construction and prediction now use the shared
  ...
  Named formula calls no longer depend on argument order or send unevaluated
  formulas to MPI workers.
...
* Kernel-sum formulas and named unconditional density/distribution formulas
  ...
  transformations no longer cause worker lookup failures or leave the master
  waiting after such failures. Numerical work retains its existing MPI routes.
...
* MPI conditional-gradient and quantile level/gradient bootstraps now share
  one original-data center across worker tasks, avoiding discarded repeated
  fits. Replicate counts, seeds, task ownership and collected results retain
  their existing contracts; no scheduler or native arithmetic is changed.
...
* Identified post-preparation local-row failures in the conditional native
  owner now agree across MPI ranks and complete owned cleanup before raising
  the original error. This does not cover pre-loop preparation failures,
  arbitrary native collective unwinds, or rank/transport loss.
...
* The internal single-index first-start restoration control is documented;
  npRmpi also transports caller-local values of that control.
...
* Copula density helpers now translate the public normalized ordered
  ...
  kernel names, defaults, or MPI orchestration.
...
* Regression bandwidth search now preserves terminal invalidity through
  ...
  CVKS; penalty multipliers must be finite and strictly greater than one. Coordinator and worker evaluations use the same raw/finalization contract.
...
* Fixed local-polynomial plot bootstraps now solve count-compressed response
  ...
  construction, moment arithmetic, chunking, progress, MPI rank assignment,
  collectives, and result ordering are unchanged. Results can change for
  systems where the retired private path imposed a different ridge decision.
...
* Eligible fixed and generalized-nearest-neighbour LP mean applications now
  ...
  beta kernels, derivative operators, ridge policy, MPI collectives, and
  public interfaces are unchanged.
