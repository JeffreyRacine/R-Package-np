...
.npscoefbw_nomad_eval_subset <- function(ctx,
                                         bws,
                                         idx,
                                         maxPenalty,
                                         localize = TRUE) {
  ...
  bwv <- moment.state$kernel.bws$bandwidth[[1L]]
  bw.divisor <- if (moment.state$bandwidth.divide && any(moment.state$kernel.bws$icon))
...
.npscoefbw_eval_pool <- function(ctx,
  ...
  if (is.null(pool) || !is.list(pool) || is.null(pool$nslaves) || pool$nslaves < 1L) {
    return(.npscoefbw_nomad_eval_direct(
      ctx = ctx,
      bws = bws,
      invalid.penalty = invalid.penalty,
      penalty.multiplier = penalty.multiplier,
...
.npscoefbw_nomad_search <- function(xdat,
  ...
  collective.owner <- isTRUE(.npRmpi_autodispatch_called_from_bcast()) &&
    .npRmpi_safe_int(mpi.comm.size(1L)) > 1L
  collective.rank <- if (collective.owner)
  ...
  evaluate.bandwidth <- function(tbw,
  ...
  if (!result.raw.valid(search.result)) {
    message <- "npscoefbw NOMAD degree search did not return a raw-valid solution"
    if (collective.owner)
