...
.np_nomad_with_powell_progress <- function(degree, expr, best_record = NULL) {
  ...
  worker_silent <- isTRUE(.np_progress_bandwidth_worker_silent())
...
.np_nomad_native_r_callback_search <- function(eval.f,
  ...
  .np_nomad_capture_solver_output(.Call(
...
.np_nomad_search <- function(engine = c("nomad", "nomad+powell"),
  ...
  record_evaluation <- function(point, started, status = "ok", msg = NULL,
                                ...
                                admissible = TRUE, extra.fields = list()) {
    state$eval_id <- state$eval_id + 1L
    rec <- c(list(
      eval_id = state$eval_id,
      degree = degree,
  ...
  wrapped_eval <- function(point) {
    ...
    tryCatch({
      ...
      if (!is.null(result)) {
        ...
        extra.fields <- result[setdiff(names(result), c("objective", "degree", "num.feval", "num.feval.fast", "admissible"))]
      }
      record_evaluation(point, started, objective = objective, degree = degree,
                        num.feval = num.feval, num.feval.fast = num.feval.fast,
                        admissible = admissible, extra.fields = extra.fields)
    }, interrupt = function(e) {
      state$interrupted <- TRUE
  ...
  run_nomad_solver <- function(start) {
    ...
    if (isTRUE(state$native.r.bridge)) {
      ...
      run.native <- function() {
        .np_nomad_native_r_callback_search(
          eval.f = native.eval,
          x0 = start,
          bbin = bbin,
      ...
      if (.npRmpi_mpi_interrupt_defer_on_master()) {
        completed.call <- NULL
        native.call <- tryCatch(
          suspendInterrupts({
            completed.call <- run.native()
            completed.call
          }),
          interrupt = function(e) {
            if (is.null(completed.call))
              stop(e)
            .npRmpi_mpi_interrupt_set_pending()
            completed.call
          }
        )
      } else {
        native.call <- run.native()
      }
