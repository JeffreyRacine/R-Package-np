...
#  if(!is.null(knots)) nbreak <- length(knots)
  ...
  ## the boundary.
	...
	attr(B, "class") <- c("np_gsl.bs","gsl.bs","matrix")
...
.np_predict_gsl_bs_impl <- function(object,
											 newx=NULL,
											 ...) {
...
predict.np_gsl.bs <- function(object,
														 newx=NULL,
														 ...) {
		.np_predict_gsl_bs_impl(object = object, newx = newx, ...)
}
