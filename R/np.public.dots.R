...
.np_public_dots_extras <- list(
  ...
  npudensbw = c("mads.nmulti", "nomad.nmulti", "nomad.remin",
                ".beta.range.certify", ".np.formula.state"),
  npudens = c("mads.nmulti", "nomad.nmulti", "nomad.remin",
              ".beta.range.certify", ".np_fit_progress_handoff", ".np.formula.state"),
  npcdensbw = c("random.seed", "mads.nmulti", ".np.formula.state"),
  npcdens = c("random.seed", "mads.nmulti", ".np_fit_progress_handoff", ".np.formula.state",
              ".np_categorical_effects", ".np_lp_first_se_demand",
              ".np_conditional_cat_se_demand", ".np.require.complete", ".np.defer.empty.rows"),
  npudistbw = c("mads.nmulti", "nomad.nmulti", "nomad.remin",
                ".beta.range.certify", ".np.formula.state"),
  npudist = c("mads.nmulti", "nomad.nmulti", "nomad.remin",
              ".beta.range.certify", ".np_fit_progress_handoff", ".np.formula.state"),
  npcdistbw = c("random.seed", "mads.nmulti", ".np.formula.state"),
  npcdist = c("random.seed", "mads.nmulti", ".np_fit_progress_handoff", ".np.formula.state",
              ".np_lp_first_se_demand", ".np_conditional_cat_se_demand", ".np.require.complete", ".np.defer.empty.rows"),
  npscoefbw = ".np.formula.state",
  npscoef = c(".np_fit_progress_allow", ".np_fit_progress_handoff", ".np.formula.state"),
  npindexbw = "remin",
  npindex = c("remin", "warn.glp.gradient", ".np_fit_progress_handoff",
              ".np_index_explicit_bws", ".np_lc_fixed_progress_route", ".np.defer.empty.rows",
              "gradient.order"),
  npplregbw = c("random.seed", "remin", ".np.formula.state"),
  npplreg = c("random.seed", "remin", ".np_fit_progress_handoff",
              ".np.defer.empty.rows", ".np.formula.state"),
