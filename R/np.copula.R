...
npcopula.formula <- function(bws,
  ...
  npcopula.default(
    ...
    u = prepared$u,
    target = target,
    evaluation = if (is.null(prepared$u)) "sample" else "grid",
    neval = neval,
    n.quasi.inv = n.quasi.inv,
    er.quasi.inv = er.quasi.inv,
    u.auto = prepared$u.auto,
    se = se
  )
}
...
.npcopula_eval <- function(bws,
  ...
  if(is.null(u)) {
    ...
    for (j in seq_len(num.var)) {
      ...
      u[,j] <- fitted(.npRmpi_with_local_regression(npudist(bws = bws.F.marginal, tdat = tdat)))
      if(density) {
        bws.f.marginal <- .npcopula_marginal_bw(bws, data, j, target = "density")
        copula <- copula/NZD(fitted(.npRmpi_with_local_regression(npudens(bws = bws.f.marginal, tdat = tdat))))
    ...
    for (j in seq_len(num.var)) {
      ...
      bws.F.marginal <- .npcopula_marginal_bw(bws, data, j, target = "distribution")
      F <- fitted(.npRmpi_with_local_regression(npudist(tdat = tdat,
                          edat = edat,
                          bws = bws.F.marginal)))
      u[u[,j] < min(F), j] <- min(F)
      u[u[,j] > max(F), j] <- max(F)
