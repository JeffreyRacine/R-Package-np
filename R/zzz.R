...
.onLoad <- function (lib, pkg) {
  ...
  if (nzchar(Sys.getenv("NP_RMPI_SKIP_INIT"))) {
    ...
    if (is.null(getOption("np.plot.inid.ksum.fastpath.nprmpi")))
      options(np.plot.inid.ksum.fastpath.nprmpi = FALSE)
    if (is.null(getOption("np.plot.bootstrap.mpi.applylb.enable")))
      options(np.plot.bootstrap.mpi.applylb.enable = FALSE)
