...
.onLoad <- function (lib, pkg) {
  ...
  if (nzchar(Sys.getenv("NP_RMPI_SKIP_INIT"))) {
    ...
    if (is.null(getOption("np.plot.bootstrap.mpi.mode")))
      options(np.plot.bootstrap.mpi.mode = "auto")
    if (is.null(getOption("np.plot.bootstrap.mpi.auto.min.n")))
      options(np.plot.bootstrap.mpi.auto.min.n = 2000L)
    if (is.null(getOption("np.plot.bootstrap.mpi.auto.min.B")))
      options(np.plot.bootstrap.mpi.auto.min.B = 256L)
    if (is.null(getOption("np.plot.bootstrap.mpi.auto.min.B.per.worker")))
      options(np.plot.bootstrap.mpi.auto.min.B.per.worker = 64L)
    if (is.null(getOption("np.plot.bootstrap.mpi.auto.min.tasks.per.worker")))
      options(np.plot.bootstrap.mpi.auto.min.tasks.per.worker = 1.0)
    if (is.null(getOption("np.plot.inid.ksum.fastpath.nprmpi")))
      options(np.plot.inid.ksum.fastpath.nprmpi = TRUE)
    if (is.null(getOption("np.plot.bootstrap.mpi.allow.applylb")))
      options(np.plot.bootstrap.mpi.allow.applylb = FALSE)
