...
parse_args <- function(args) {
  out <- list(
    ...
    seeds = NULL,
    nslaves = 1L,
    out_raw = "/tmp/npcdens_mpi_bench_raw.csv",
    out_summary = "/tmp/npcdens_mpi_bench_summary.csv",
    show_progress = TRUE
  )
    ...
    run = {
      ...
      out <- tryCatch({
        ...
        eval(substitute(
          ...
          list(S = s, N = cfg$n, BWM = cfg$bwmethod, NM = cfg$nmulti, CKX = cfg$cxkertype, CKY = cfg$cykertype)
        ))
...
main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  if (cfg$show_progress) {
    cat("Running npcdens npRmpi benchmark with", length(seeds), "seeds\n")
    cat("backend=npRmpi",
        " n=", cfg$n,
        " nslaves=", cfg$nslaves,
