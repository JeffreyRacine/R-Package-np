...
parse_args <- function(args) {
  out <- list(
    ...
    seeds = NULL,
    nslaves = 1L,
    out_raw = "/tmp/npcdist_mpi_bench_raw.csv",
    out_summary = "/tmp/npcdist_mpi_bench_summary.csv",
    show_progress = TRUE
  )
  ...
  for (a in args) {
    ...
    else if (key == "bwtype") out$bwtype <- val
    else if (key == "cxkertype") out$cxkertype <- val
    else if (key == "cykertype") out$cykertype <- val
    else if (key == "ckerpair") { out$cxkertype <- val; out$cykertype <- val }
  ...
  if (!out$bwmethod %in% c("cv.ls", "normal-reference")) stop("bwmethod must be cv.ls or normal-reference")
  if (!is.finite(out$nmulti) || out$nmulti < 1L) stop("nmulti must be an integer >= 1")
  if (!out$bwtype %in% c("fixed", "generalized_nn", "adaptive_nn")) stop("bwtype must be fixed, generalized_nn, or adaptive_nn")
  if (!out$cxkertype %in% c("gaussian", "epanechnikov")) stop("cxkertype must be gaussian or epanechnikov")
  if (!out$cykertype %in% c("gaussian", "epanechnikov")) stop("cykertype must be gaussian or epanechnikov")
  if (out$cxkertype != out$cykertype) stop("cxkertype and cykertype must match for paired-kernel comparisons")
...
run_case <- function(seed, cfg) {
  ...
  bw_str <- function(bw) {
    ...
    if (!is.null(bw$bandwidth) && length(bw$bandwidth) > 0L) {
      ...
      vals <- c(suppressWarnings(as.numeric(bw$ybw)), suppressWarnings(as.numeric(bw$xbw)))
    }
  ...
  first_num_multi <- function(...) {
    xs <- list(...)
    for (x in xs) {
      if (!is.null(x) && length(x) > 0L) return(as.numeric(x)[1])
    }
    NA_real_
  }
    ...
    run = {
      ...
      out <- tryCatch({
        ...
        eval(substitute(
          ...
          list(S = s, N = cfg$n, BWM = cfg$bwmethod, NM = cfg$nmulti, BWT = cfg$bwtype, CKX = cfg$cxkertype, CKY = cfg$cykertype)
        ))
        ...
        list(
          ...
          log_likelihood = if (!is.null(fit$condist) && length(fit$condist) > 0L) mean(as.numeric(fit$condist), na.rm = TRUE) else NA_real_,
          bw = bw_str(bw),
          error = ""
        )
      }, error = function(e) {
  ...
  data.frame(
    ...
    bwtype = rep(cfg$bwtype, cfg$times),
    cxkertype = rep(cfg$cxkertype, cfg$times),
    cykertype = rep(cfg$cykertype, cfg$times),
...
summarize_results <- function(df) {
  ...
  data.frame(
    ...
    bwtype = okdf$bwtype[1],
    cxkertype = okdf$cxkertype[1],
    cykertype = okdf$cykertype[1],
...
main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  if (cfg$show_progress) {
    cat("Running npcdist npRmpi benchmark with", length(seeds), "seeds\n")
    cat("backend=npRmpi",
        " n=", cfg$n,
        " nslaves=", cfg$nslaves,
        ...
        " nmulti=", cfg$nmulti,
        " bwtype=", cfg$bwtype,
        " cxkertype=", cfg$cxkertype,
        " cykertype=", cfg$cykertype,
        " np_tree=", cfg$np_tree,
