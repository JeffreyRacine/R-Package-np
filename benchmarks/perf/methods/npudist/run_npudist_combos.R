...
main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  script_path <- if (!is.na(script_arg) && nzchar(script_arg)) sub("^--file=", "", script_arg) else file.path(getwd(), "run_npudist_combos.R")
  here <- dirname(normalizePath(script_path))

  bench_script <- file.path(here, "bench_npudist_param_nprmpi.R")
  if (!file.exists(bench_script)) stop("Missing benchmark script: ", bench_script)

  hash <- repo_hash(here)
  run_id <- mk_run_id(cfg, "npRmpi", hash)

  out_raw <- file.path(cfg$out_dir, paste0("npudist_combo_raw_", run_id, ".csv"))
  out_summary <- file.path(cfg$out_dir, paste0("npudist_combo_summary_", run_id, ".csv"))
  out_manifest <- file.path(cfg$out_dir, paste0("npudist_combo_manifest_", run_id, ".csv"))
  ...
  for (i in seq_len(combo_n)) {
    ...
    if (cfg$show_progress) {
      cat(sprintf("[%02d/%02d] bw=%s bwtype=%s cker=%s tree=%s seed_policy=%s\n",
                  i, nrow(combos), row$bwmethod, row$bwtype, row$ckertype,
    ...
    combo_log <- file.path(cfg$out_dir, sprintf("npudist_combo_%s_%02d.log", run_id, i))
    env_vec <- c(
