...
parse_args <- function(args) {
  out <- list(
    ...
    seeds = NULL,
    nslaves = 1L,
    out_raw = "/tmp/npscoef_mpi_bench_raw.csv",
    out_summary = "/tmp/npscoef_mpi_bench_summary.csv",
    show_progress = TRUE
  )
...
main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  if (cfg$show_progress) {
    cat("Running npscoef npRmpi benchmark with", length(seeds), "seeds\n")
    cat("backend=npRmpi",
        " n=", cfg$n,
        " nslaves=", cfg$nslaves,
        " nmulti=", cfg$nmulti,
