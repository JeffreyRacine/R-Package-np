...
parse_args <- function(args) {
  out <- list(
    ...
    base_seed = 42L,
    seeds = NULL,
    out_raw = "/tmp/npindex_serial_bench_raw.csv",
    out_summary = "/tmp/npindex_serial_bench_summary.csv",
    show_progress = TRUE
  )
...
mk_data <- function(seed, n, method) {
  set.seed(seed)
...
run_case <- function(seed, cfg) {
  ...
  mb <- microbenchmark::microbenchmark(
    run = {
      ...
      s <- iter_seeds[[idx]]
      out <- tryCatch({
        dat <- mk_data(s, cfg$n, cfg$method)
...
main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  if (cfg$show_progress) {
    cat("Running npindex serial benchmark with", length(seeds), "seeds\n")
    cat("backend=np",
        " n=", cfg$n,
        " method=", cfg$method,
        " nmulti=", cfg$nmulti,
