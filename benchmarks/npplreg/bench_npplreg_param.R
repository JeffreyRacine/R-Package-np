...
parse_args <- function(args) {
  out <- list(
    ...
    base_seed = 42L,
    seeds = NULL,
    out_raw = "/tmp/npplreg_serial_bench_raw.csv",
    out_summary = "/tmp/npplreg_serial_bench_summary.csv",
    show_progress = TRUE
  )
...
mk_data <- function(seed, n) {
  ...
  z2 <- rnorm(n)
  y <- 1 + x1 + as.numeric(as.character(x2)) + as.numeric(as.character(z1)) + sin(z2) + rnorm(n)
    ...
    run = {
      ...
      out <- tryCatch({
        ...
        list(
          ...
          log_likelihood = first_num_or_na(fit$MSE),
          bw = bw_str(bw),
          error = ""
        )
      }, error = function(e) {
...
main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  if (cfg$show_progress) {
    cat("Running npplreg serial benchmark with", length(seeds), "seeds\n")
    cat("backend=np",
        " n=", cfg$n,
        " nmulti=", cfg$nmulti,
        " ckertype=", cfg$ckertype,
