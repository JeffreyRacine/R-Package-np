...
parse_args <- function(args) {
  out <- list(
    ...
    label_a = "A",
    label_b = "B",
    out_timing = "/tmp/npcdist_version_timing_compare.csv",
    out_objective = "/tmp/npcdist_version_objective_compare.csv",
    out_combo_timing = "/tmp/npcdist_version_combo_timing_compare.csv",
    out_combo_objective = "/tmp/npcdist_version_combo_objective_compare.csv"
  )
  for (a in args) {
...
combo_keys <- function(df) {
  keep <- c("n", "bwmethod", "nmulti", "bwtype", "cxkertype", "cykertype", "np_tree", "seed_policy")
  keep[keep %in% names(df)]
}

match_keys <- function(a, b) {
  keep <- c("n", "bwmethod", "nmulti", "bwtype", "cxkertype", "cykertype", "np_tree", "seed_policy", "seed", "iter")
...
summ_timing_by_combo <- function(df, label) {
  ...
  rows <- lapply(parts, function(g) {
    data.frame(
      ...
      bwtype = if ("bwtype" %in% names(g)) g$bwtype[1] else "",
      cxkertype = g$cxkertype[1],
      cykertype = g$cykertype[1],
      np_tree = g$np_tree[1],
...
mk_timing <- function(df, label) {
  data.frame(
    function_name = c("npcdistbw", "npcdist", "npcdist_total"),
...
main <- function() {
  ...
  combo_keys_common <- intersect(
    c("n", "bwmethod", "nmulti", "bwtype", "cxkertype", "cykertype", "np_tree", "seed_policy"),
  ...
  combo_timing <- data.frame(
    ...
    bwtype = getcol(combo_m, "bwtype"),
    cxkertype = getcol(combo_m, "cxkertype"),
    cykertype = getcol(combo_m, "cykertype"),
  ...
  cks <- combo_keys(m)
  if (length(cks) == 0L) cks <- intersect(c("n", "bwmethod", "bwtype", "cxkertype", "cykertype", "np_tree", "seed_policy"), names(m))
  ...
  combo_obj_rows <- lapply(grp, function(g) {
    data.frame(
      ...
      nmulti = if ("nmulti" %in% names(g)) g$nmulti[1] else NA_integer_,
      bwtype = if ("bwtype" %in% names(g)) g$bwtype[1] else "",
      cxkertype = if ("cxkertype" %in% names(g)) g$cxkertype[1] else "",
      cykertype = if ("cykertype" %in% names(g)) g$cykertype[1] else "",
