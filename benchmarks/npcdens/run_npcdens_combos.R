...
main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  script_path <- if (!is.na(script_arg) && nzchar(script_arg)) sub("^--file=", "", script_arg) else file.path(getwd(), "run_npcdens_combos.R")
  here <- dirname(normalizePath(script_path))

  bench_script <- file.path(here, "bench_npcdens_param.R")
  if (!file.exists(bench_script)) stop("Missing benchmark script: ", bench_script)

  hash <- repo_hash(here)
  run_id <- mk_run_id(cfg, "np", hash)

  out_raw <- file.path(cfg$out_dir, paste0("npcdens_combo_raw_", run_id, ".csv"))
  out_summary <- file.path(cfg$out_dir, paste0("npcdens_combo_summary_", run_id, ".csv"))
  out_manifest <- file.path(cfg$out_dir, paste0("npcdens_combo_manifest_", run_id, ".csv"))
  ...
  for (i in seq_len(nrow(combos))) {
    ...
    if (cfg$show_progress) {
      cat(sprintf("[%02d/%02d] bw=%s ckerpair=%s tree=%s seed_policy=%s\n",
                  i, nrow(combos), row$bwmethod, row$ckerpair,
