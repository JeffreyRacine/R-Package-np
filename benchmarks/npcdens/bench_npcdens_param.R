...
parse_args <- function(args) {
  out <- list(
    ...
    base_seed = 42L,
    seeds = NULL,
    out_raw = "/tmp/npcdens_serial_bench_raw.csv",
    out_summary = "/tmp/npcdens_serial_bench_summary.csv",
    show_progress = TRUE
  )
  ...
  if (!out$bwmethod %in% c("cv.ml", "cv.ls")) stop("bwmethod must be cv.ml or cv.ls")
    ...
    run = {
      ...
      out <- tryCatch({
        ...
        list(
          ...
          log_likelihood = first_num_or_na(fit$log_likelihood),
          bw = bw_str(bw),
          error = ""
        )
      }, error = function(e) {
...
main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  if (cfg$show_progress) {
    cat("Running npcdens serial benchmark with", length(seeds), "seeds\n")
    cat("backend=np",
        " n=", cfg$n,
        " bwmethod=", cfg$bwmethod,
        " nmulti=", cfg$nmulti,
