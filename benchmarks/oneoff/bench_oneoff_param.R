...
parse_args <- function(args) {
  out <- list(
    ...
    base_seed = 42L,
    seeds = NULL,
    out_raw = "/tmp/np_oneoff_bench_raw.csv",
    out_summary = "/tmp/np_oneoff_bench_summary.csv",
    show_progress = TRUE
  )
...
run_one <- function(fun, n, seed) {
  ...
  tryCatch({
    if (fun == "npcmstest") {
      ...
      ans <- npcmstest(model = model, xdat = X, ydat = d$growth, nmulti = 1L, ftol = 0.01, tol = 0.01)
