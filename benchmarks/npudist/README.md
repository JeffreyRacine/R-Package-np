# npudist Benchmark Harness (np)

This folder provides a parameterized benchmark harness for `npudistbw()` + `npudist()` in serial `np` using `microbenchmark`.

## Files

- `bench_npudist_param.R`: run one benchmark configuration with `microbenchmark` repetitions.
...
Rscript /Users/jracine/Development/np-master/benchmarks/npudist/bench_npudist_param.R \
  ...
  --out_raw=/tmp/npudist_one_raw.csv --out_summary=/tmp/npudist_one_summary.csv
```
