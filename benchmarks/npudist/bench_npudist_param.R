...
parse_args <- function(args) {
  out <- list(
    ...
    base_seed = 42L,
    seeds = NULL,
    out_raw = "/tmp/npudist_serial_bench_raw.csv",
    out_summary = "/tmp/npudist_serial_bench_summary.csv",
    show_progress = TRUE
  )
  ...
  if (!out$bwmethod %in% c("cv.cdf", "normal-reference")) stop("bwmethod must be cv.cdf or normal-reference")
...
run_case <- function(seed, cfg) {
  ...
  bw_str <- function(bw) {
    vals <- NULL
    if (!is.null(bw$bw) && length(bw$bw) > 0L) {
      vals <- suppressWarnings(as.numeric(unlist(bw$bw)))
    } else if (!is.null(bw$bandwidth) && length(bw$bandwidth) > 0L) {
    ...
    run = {
      ...
      out <- tryCatch({
        ...
        list(
          ...
          log_likelihood = if (!is.null(fit$dist) && length(fit$dist) > 0L) mean(as.numeric(fit$dist), na.rm = TRUE) else NA_real_,
          bw = bw_str(bw),
          error = ""
        )
      }, error = function(e) {
...
main <- function(args = commandArgs(trailingOnly = TRUE)) {
  ...
  if (cfg$show_progress) {
    cat("Running npudist serial benchmark with", length(seeds), "seeds\n")
    cat("backend=np",
        " n=", cfg$n,
        " bwmethod=", cfg$bwmethod,
        " nmulti=", cfg$nmulti,
