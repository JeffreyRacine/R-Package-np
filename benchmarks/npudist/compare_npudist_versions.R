...
parse_args <- function(args) {
  out <- list(
    ...
    label_a = "A",
    label_b = "B",
    out_timing = "/tmp/npudist_version_timing_compare.csv",
    out_objective = "/tmp/npudist_version_objective_compare.csv",
    out_combo_timing = "/tmp/npudist_version_combo_timing_compare.csv",
    out_combo_objective = "/tmp/npudist_version_combo_objective_compare.csv"
  )
  for (a in args) {
...
combo_keys <- function(df) {
  keep <- c("n", "bwmethod", "nmulti", "bwtype", "ckertype", "np_tree", "seed_policy")
  keep[keep %in% names(df)]
}

match_keys <- function(a, b) {
  keep <- c("n", "bwmethod", "nmulti", "bwtype", "ckertype", "np_tree", "seed_policy", "seed", "iter")
...
mk_timing <- function(df, label) {
  data.frame(
    function_name = c("npudistbw", "npudist", "npudist_total"),
...
main <- function() {
  ...
  combo_keys_common <- intersect(
    c("n", "bwmethod", "nmulti", "bwtype", "ckertype", "np_tree", "seed_policy"),
  ...
  cks <- combo_keys(m)
  if (length(cks) == 0L) cks <- intersect(c("n", "bwmethod", "bwtype", "ckertype", "np_tree", "seed_policy"), names(m))
