%% $Id: np_faq.tex,v 1.79 2013/03/06 18:36:31 jracine Exp jracine $
...
\section*{Changes from Version 0.40-13 to 0.50-1 [06-Mar-2013]}

\begin{itemize}

\item The functions \texttt{npudist()} and \texttt{npudistbw()} now
  uncoupled from \texttt{npudens()} and \texttt{npudensbw()}
...
\item Fixed glitch in gradient standard errors in npindex where
  identical standard errors were output in \texttt{model\$gerr}
...
\item Added reference to R2 measure (Doksum and Samarov (1995))
