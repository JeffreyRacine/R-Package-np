%% $Id: np_faq.tex,v 1.73 2012/05/22 01:53:12 jracine Exp jracine $
...
\title{Package {\tt np} FAQ} \date{\today} \author{Jeffrey S.~Racine}
...
is 0.40-13. Kindly update your version should you not be using the most
current (from within R, {\tt update.packages()} ought to do it, though
also see \ref{update} below.). See the appendix in this file for
cumulative changes between this and previous versions of the {\tt np}
package.

\section{Frequently Asked Questions}

\subsection{How do I cite the {\tt np} package?}

Once you have installed the {\tt np} package ({\tt
  install.packages("np")}), if you load the {\tt np} package ({\tt
...
\subsection{I have never used {\tt R} before. Can you direct me to some
  ...
  the {\tt np} package are directed to the {\tt Ecdat} \cite{Ecdat}
...
\subsection{Is there a `gentle guide' to the {\tt np} package that
    contains some easy to follow examples?}

  Perhaps the most gentle introduction is contained in the {\tt np}
  package itself in the form of a `vignette'. To view the vignette run
  R, install the {\tt np} package ({\tt install.packages("np")}), then
  type {\tt vignette("np",package="np")} to view or print the vignette
  (this vignette is essentially the article that appeared in the
  Journal of Statistical Software that describes the {\tt np} package
  ...
  For a listing of all routines in the {\tt np} package type: {\tt
    library(help="np")}.

\subsection{I noticed you have placed a new version of the {\tt np}
...
\subsection{What is the difference between the {\tt np} package and the
    previous stand-alone programs you wrote, namely, N \copyright,
    NPREG \copyright, and NPDEN \copyright?}

  The {\tt np} package is built from the same C library that underlies
...
\subsection{I want to use so-and-so's semiparametric/nonparametric
    method, however, the {\tt np} package does not include this
  ...
  advantage of underlying kernel methods implemented in the {\tt np}
  ...
  the {\tt np} package, the best way to achieve this is to
...
\subsection{Cross-validation takes forever, and I can't wait that
  ...
  \item Some background may be in order. Cross-validation methods have
    ...
    minor differences in how {\tt np} functions are called, and you
  ...
  \item Alternatively, you can use the method outlined in Racine
    ...
    perhaps simpler to implement using the {\tt np} package).
...
\end{verbatim}
    \end{singlespacing}
    then reinstall the {\tt np} package and you may experience some
...
\end{verbatim}
  ...
  univariate density estimation routines in {\tt np} is
...
\subsection{\label{code_nowork}I wrote a program using {\tt np} and it
  ...
  If you are convinced that the problem lies with {\tt np} (there
...
\subsection{When some routines are running under MS Windows, {\tt R}
  ...
  GUI, and was not specific to the {\tt np} package.
...
\subsection{\label{factor}A variable must be cast as, say, a factor in
    order for {\tt np} to recognize this as an unordered factor. How
...
\subsection{My {\tt np} code produces errors when I attempt to run
    it\dots}
...
\subsection{\label{0/1}I have (numeric) 0/1 dummy variable regressors
    in my parametric model. Can I just pass them to the {\tt np}
  ...
  In {\tt np} (and {\tt R} in general), you get to economize by just
  ...
  {\tt np} function as you would for linear parametric models. The
  ...
  enter this as {\tt factor(sex)} so that the {\tt np} function
...
\subsection{How can I estimate additive semiparametric models?}
  ...
  function via the \verb+wage1+ dataset included in the {\tt np}
...
\end{verbatim}
  ...
  functions in the {\tt np} package). The following code snippet
...
\bibliographystyle{plain} \bibliography{np_faq}
...
\item Added a new function {\tt npregiv} that conducts nonparametric
  ...
  $\varphi(z)$ (i.e.\ lacks much functionality relative to other {\tt
    np} functions) and is in `beta status' until further notice.
