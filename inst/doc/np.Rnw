...
The predicted square error on the hold-out data was \Sexpr{pse.ols}
for the parametric linear model, \Sexpr{pse.np.freq} for the traditional
nonparametric estimator that splits the data into subsets, and
\Sexpr{pse.np} for the nonparametric estimator that uses the full
sample but smooths both the qualitative and quantitative data. The
nonparametric model that smooths both the quantitative and qualitative
data is more than \Sexpr{(1-round(pse.np/pse.ols))*100}\% more
efficient in terms of out-of-sample predictive ability than the
