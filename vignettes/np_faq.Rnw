...
\end{verbatim}
this typically means the MPI session has not been started.
...
\item For batch jobs launched with \texttt{mpiexec -n ...},
  call \texttt{npRmpi.init(mode="attach")} near the top of your script.
  This replaces the legacy external profile bootstrap workflow.
