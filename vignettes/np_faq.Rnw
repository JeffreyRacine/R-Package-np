%% $Id: np_faq.tex,v 1.88 2014/03/18 18:03:25 jracine Exp jracine $
...
linear estimator is obtained by minimizing (consider the one-predictor
case to fix ideas)
\begin{equation*}
...
g(x)=\hat a$ with respect to $x$) is \textsl{not} $\hat b$ (if you compute
the partial algebraically you will see they are not the same), unless
the bandwidth is very large (i.e.\ $h=\infty$).
