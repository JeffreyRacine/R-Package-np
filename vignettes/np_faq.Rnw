%% $Id: np_faq.tex,v 1.94 2015/07/01 09:00:52 jracine Exp jracine $
...
demonstrates; see also \cite{KARUNAMUNIA_ZHANG:2008} for improved data
...
Certainly. The following conducts kernel regression with exclusively
binary predictors by way of illustration.
\begin{verbatim}
...
## this function is much faster than that ignoring the sparse
## structure (e.g., the function npreg() currently does not attempt to
...
cv.ls.uniquecombs <- function(lambda,y,z,z.unique,ind,ind.vals,num.z,c) {
    ...
    for(i in 1:c) {
        ...
        L <- apply(sapply(1:num.z, function(j){kernel(z[,j],
                                                      ...
                                                      lambda[j])}),1,prod)
        g[zz] <- (sum(y*L)-(y*L)[zz])/(sum(L)-L[zz])
    }
    return(mean((y-g)^2))
}
...
library(np)
t.npreg <- system.time(g.npreg <- npreg(y~factor(z[,1])+
                                          factor(z[,2])+
                                          factor(z[,3])+
                                          factor(z[,4]),
                                          ukertype="liracine",
