...
is 0.60-11. Kindly update your version should you not be using the most
...
\section*{Changes from Version 0.60-10 to 0.60-11 [20-Oct-2020]}

\begin{itemize}

\item \texttt{npuniden.boundary()} uses empirical support for a and b
  (previously was [0,1], now [min(X),max(X)])
