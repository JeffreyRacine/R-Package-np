...
is 0.60-20. Kindly update your version should you not be using the most
...
\subsection{I noticed you have placed a new version of the \texttt{np}
  ...
  (\url{https://CRAN.R-project.org/package=np/ChangeLog}), or go
...
at \url{https://jeffreyracine.github.io/gallery/}.
...
\subsection{Under Mac OS X, when I run a command no progress is
  ...
  *NIX\footnote{*NIX is often used to describe UNIX and other
  ...
  prefer to existing interfaces (\url{https://posit.co/}).
...
\subsection{When I use \texttt{plot()} existing graphics
  ...
  Alternately, you can use RStudio (\url{https://posit.co/})
...
\subsection{I specify a variable as \texttt{factor()} or \texttt{ordered()}
  ...
  This arises because the \texttt{plot()} method tries to retrieve
...
\subsection{\label{partial grad q}When I estimate my gradients and
  ...
  The \texttt{plot()} methods plot 'partial'
  ...
  form the evaluation data for $x_1$ (unlike the default \texttt{plot()} behavior which
...
\subsection{I use \texttt{plot()} to plot, say, a density
  ...
  continuous data, among others. By default, \texttt{plot()} takes
...
Two common failure signatures are
\begin{verbatim}
...
\end{verbatim}
The first means no active MPI session; the second usually means a
manual-broadcast/profile script started without loading the required
\texttt{.Rprofile}.

\begin{enumerate}

\item For interactive sessions, call
  \texttt{npRmpi.init(mode="spawn", nslaves=...)} first.

\item For batch jobs launched with \texttt{mpiexec -n ...} and
  auto-dispatch scripts, call \texttt{npRmpi.init(mode="attach")} near
  the top of your script and launch with
  \texttt{Rscript --no-save} (or \texttt{R CMD BATCH --no-save}).

\item For manual-broadcast/profile scripts (explicit
  \texttt{mpi.bcast.cmd(...)} calls), set
  \texttt{R\_PROFILE\_USER=<path-to-.Rprofile>} and clear
  \texttt{R\_PROFILE}. Launch with
  \texttt{R CMD BATCH --no-save} and do not use
  \texttt{R CMD BATCH --vanilla}.

\item Enable auto-dispatch if desired with
  \texttt{options(npRmpi.autodispatch=TRUE, np.messages=FALSE)}.
...
\section*{Changes from Version 0.60-19 to 0.60-20 [06-Feb-2026]}

\begin{itemize}

\item Fixed np issues 4/6/7/13/18/26/51 and added np.pairs utilities

\item Merged all bug fixes and modifications present in the np package since the previous update of the npRmpi package (0.60-2 [27-Jun-2014]) in to the npRmpi package, updated MPI functionality to reflect those in the most recent Rmpi version (0.7-3.3).

\item Major enhancements to npregiv() and npregivderiv():

\begin{itemize}

  \item Implemented warm-start strategy for improved convergence
  \item Aligned multistart strategy with crsiv/crsivderiv for consistency
  \item Performance optimizations: pre-allocated matrices/lists in iterative loops,
    optimized trapezoidal integration, improved Tikhonov and Ridge calculations
  \item Added S3 methods (print, summary, plot) for better integration with R workflow
  \item Enhanced summaries with bandwidth details and estimation timing information
  \item Alphabetized function arguments for consistency
  \item Fixed initialization of bw.mu.w in npregivderiv
  \item Cleaned up naming convention (phihat to phi throughout)
...
\begin{itemize}
  ...
  \item Added comprehensive unit tests using testthat framework
  \item Expanded test coverage to include distribution, copula, quantile regression,
    tests of independence/specification, and boundary/shape-constrained methods

\end{itemize}

\item Build and maintenance updates:

\begin{itemize}

  \item Refined .Rbuildignore regexes to correctly exclude .git and build artifacts
  \item Added missing imports (graphics, grDevices) to NAMESPACE to resolve global
    function notes
  \item Restored dontrun blocks in documentation to expedite package checks
  \item Suppressed build warnings for non-standard man directory files
...
\begin{itemize}
  ...
  \item Fixed np.plot categorical predictor evaluation
  \item Fixed np package registrations after sync
  \item Synced non-MPI code fixes from npRmpi

\end{itemize}

\end{itemize}

\section*{Changes from Version 0.60-29 to 0.60-20 [11-Feb-2026]}
...
\begin{itemize}
  \item Resolved -Wfree-nonheap-object compiler warnings in nr.c by implementing 
    a safe 0-based allocation strategy for 1-based indexing compatibility
  \item Refined .Rbuildignore regexes to correctly exclude .git and build artifacts
  \item Added missing imports (graphics, grDevices) to NAMESPACE to resolve global
    function notes
  \item Restored \texttt{dontrun} blocks in documentation to expedite package checks
  \item Suppressed build warnings for non-standard man directory files
\end{itemize}  
...
\begin{itemize}
  \item Updated URLs to resolve CRAN check warnings
  \item Updated McMaster website URL to https://experts.mcmaster.ca/people/racinej
  \item Added build notes, worktrees documentation, and CRAN diff summary
  \item Fixed documentation mismatches for npregiv/npregivderiv
\end{itemize}  

\end{itemize}

\section*{Changes from Version 0.60-18 to 0.60-19 [28-Jan-2026]}

\begin{itemize}

\item Added crs to Suggests field in DESCRIPTION per CRAN requirements

\item Utility functions \texttt{integrate.trapezoidal()}, \texttt{NZD()}, \texttt{NZD\_pos()} (new, affect npregiv() and npregivderiv())

\item Miscellaneous typos fixed

\end{itemize}

\section*{Changes from Version 0.60-17 to 0.60-18 [09-Dec-2024]}

\begin{itemize}

\item Patched issue with \texttt{as.double(data)} when data is a NULL object
  reported by Brian Ripley

\item Merged rng pull request \texttt{https://github.com/JeffreyRacine/R-Package-np/pull/48}

\item Incorporated Kurt Hornik's kind suggestion ``If indeed intended as a
  package overview help file, please add \texttt{{\textbackslash}alias\{np-package\}} to file
  \texttt{'np/man/np-package.Rd'}''
...
\begin{verbatim}
  Check: S3 generic/method consistency Result: WARN
  compute.bootstrap.errors: function(..., bws)
  compute.bootstrap.errors.*} triggered potentially by lack of
  \texttt{...,bw} in functions called
\end{verbatim}

(thanks to Kurt Hornik for assistance!)

\item Updated \texttt{inst/CITATIONS} to get rid of old-style cit and
  bib entries
...
\item Added support for character expansion in plotting methods used by \texttt{plot()} on \texttt{np} objects; defaults are taken from \texttt{par()} and can be set directly (e.g., \texttt{plot(foo,cex.main=0.8)}).
...
\item \texttt{plot()} uses class-specific S3 plotting methods and plotting
...
\item Added option \texttt{plot.par.mfrow=TRUE/FALSE} to
  \texttt{plot()} so that the automated setting of
...
\item Fixed issue where calling \texttt{plot} resets system seed

\item Updated examples in docs so that plot is recommended throughout
  (and not \texttt{plot} that is invoked by \texttt{plot})
...
\item Fixed code regression (dating to version 0.30-4) where
  ...
  \texttt{npsymtest}, \texttt{npunitest}, and \texttt{plot}
...
\item \texttt{plot()} now returns data upon request for conditional
  densities
...
\item Fixed a bug in \texttt{plot()} with partially linear regressions
...
\item Fixed bug where incorrect asymptotic standard errors of
  gradients for regression objects were being displayed in \texttt{plot()}
