...
\begin{verbatim}
> npregbw(y~x)
Error: npRmpi auto-dispatch requires an active MPI slave pool; call npRmpi.start(...) first
\end{verbatim}
...
\item For interactive sessions, call
  \texttt{npRmpi.start(mode="spawn", nslaves=...)} first.

\item For batch jobs launched with \texttt{mpiexec -n ...},
  call \texttt{npRmpi.start(mode="attach")} near the top of your script.
