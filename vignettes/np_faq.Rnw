%% $Id: np_faq.tex,v 1.89 2014/04/17 05:36:22 jracine Exp jracine $
...
is 0.60-1. Kindly update your version should you not be using the most
...
\subsection{Cross-validation takes forever, and I can't wait that
  ...
  \item Some background may be in order. Cross-validation methods have
    ...
    version of the \texttt{np} package titled `\texttt{npRmpi}' exists for
    running jobs in a parallel environment that leverages the \texttt{Rmpi} package (\cite{Rmpi}).\footnote{We are extremely grateful
    ...
    local advice from your sysadmin or others. The document \texttt{npRmpi.pdf} may contain some useful information for such
    ...
    ought to then be trivial to install the \texttt{npRmpi} package. Also
    ...
    and the overview file \texttt{npRmpi.pdf} in the inst/doc/ directory of
    the \texttt{npRmpi} package you downloaded and unpacked.
...
\end{singlespacing}
  ...
  \item Barring this, you can set the search tolerances to be a bit
    ...
    trapped in local minima) by setting, say, \texttt{tol=0.1} and \texttt{ftol=0.1} in the respective bandwidth routine (see the docs for
    examples). Also, you can set \texttt{nmulti=1} which overrides the
...
\end{verbatim}
  ...
  \item Finally, I have recently been working on extending regression
    ...
    predictors (see references for papers with Yang and Ma in the \texttt{crs} package). Because regression splines involve simple
...
\subsection{\label{code_nowork}I wrote a program using \texttt{np} and it
  ...
  docs. You can run these examples by typing \texttt{example("npfunctionname")} where npfunctionname is, say, \texttt{w},
  as in \texttt{example("w")}.\footnote{For a listing of all routines in
    the np package type: \texttt{library(help="np")}.} These examples
...
\subsection{Some results take a while, and my MS Windows computer is
  ...
  You can easily change the priority of your \texttt{R} job on the fly,
  just as you might under *NIX.  Pull up the task manager
  (\verb+<Ctrl>-<Alt>-<Del>+), go to the process list, and find the
  process \texttt{Rgui.exe} (or \texttt{R.exe} if you are running \texttt{Rterm}), select this process by left clicking on it, then right
  clicking will bring up a menu, select \texttt{Set priority}, then
  change priority to \texttt{low} and hit $<$\texttt{ok}$>$. For lengthy
  ...
  priority of \texttt{R} under MS Windows by modifying the properties of
  your \texttt{R} desktop icon.
...
\subsection{I specify a variable as \texttt{factor()} or \texttt{ordered()}
  ...
  To correct this, simply call \texttt{plot()} with the argument \texttt{data=dataframe} where \texttt{dataframe} is the name of your
  data frame.
...
\subsection{My \texttt{np} code produces errors when I attempt to run
  ...
  examples) as in \texttt{npregbw(formula=y\textasciitilde x)} (i.e., explicitly
  ...
  page closely (i.e., \texttt{?npreg} then scroll down to \texttt{Examples:}). See also \ref{code_nowork} above.
...
\subsection{\label{0/1}I have (numeric) 0/1 dummy variable regressors
  ...
  In \texttt{np} (and \texttt{R} in general), you get to economize by just
  ...
  variable, say `\texttt{sex}', and in this case you still would have to
  enter this as \texttt{factor(sex)} so that the \texttt{np} function
...
\end{verbatim}
  ...
  is `pathological' in that it has \texttt{IQR=0} but \texttt{std>0} (see
...
\subsection{\label{partial grad q}When I estimate my gradients and
  ...
  The function \texttt{plot()} (\texttt{npplot()}) plots `partial' means and
  ...
  form the evaluation data for $x_1$ (unlike \texttt{npplot()} which uses
  an evenly spaced grid). Note we use the function \texttt{uocquantile()}
  ...
  \texttt{uocquantile()} can compute quantiles for ordered, unordered,
  and continuous data (see \texttt{?uocquantile} for details).
...
\subsection{I use \texttt{plot()} \texttt{(npplot())} to plot, say, a density
  ...
  continuous data, among others. By default, \texttt{npplot()} takes the
...
\subsection{Can \texttt{npksum()} compute analytical derivatives with
  ...
  code snippet (this supports multiple arguments including \texttt{"integral"} and \texttt{"convolution"} in addition to \texttt{"normal"},
  the default).
...
\end{verbatim}
  ...
  \texttt{errors=TRUE} to your call to npindex so that \texttt{se(model)}
  returns the vector of standard errors for the estimated conditional
  mean where \texttt{model} is the name of your model.  Note that \texttt{model\$merr } will contain the standard errors returned by \texttt{se(model)} while \texttt{model\$gerr} will return the matrix of
  standard errors for the gradients {\em provided} you have set \texttt{gradients=TRUE} (furthermore, \texttt{model\$mean.grad} and \texttt{model\$mean.gerr} will give the average derivative and its
...
\subsection{When I run \texttt{R} in batch mode via \texttt{R CMD BATCH
  ...
  After loading the np library add the line \texttt{options(np.messages=FALSE)} and all such messages will be
  disabled.
...
\subsection{I have a large number of variables, and when using the
  ...
  The issue is that converting formulas into character strings in \texttt{R} appears to be limited to 500 characters. We are not aware of a
...
\subsection{How can I estimate additive semiparametric models?}
  ...
  the \texttt{gam} package (though they do not support categorical
  variables). The \texttt{gam} package function \texttt{gam()} (the \texttt{mgcv} package also contains a similar function by the same name)
  ...
  (\texttt{`loess', lo($\cdot$)}, with default manual `span' of 0.75, the
...
\end{verbatim}
  ...
  demonstrates the \texttt{mgcv} implementation of the \texttt{gam()}
  function.
...
\end{verbatim}
this likely means that either you have failed to place the \texttt{.Rprofile} file in the current or root directory as directed, or the
\texttt{.Rprofile} initialization code is not being loaded as expected.

\begin{enumerate}

\item Make sure a copy of the initialization file \texttt{Rprofile}
  exists in your working or root directory and is named \texttt{.Rprofile}

\item Make sure you did not run \texttt{R} with either the \texttt{--no-init-file} or \texttt{--vanilla} option (this combines a number
  of options including \texttt{--no-init-file} which will disable reading
  of \texttt{.Rprofile})
...
Try the function \texttt{ts.intersect}, which can exploit R's lag
function but return a suitable data frame, as per the following
illustration:
\begin{verbatim}
data(lynx)
loglynx <- log(lynx)
...
\section*{Changes from Version 0.60-0 to 0.60-1 [2-Jun-2014]}
...
\item Fixed issue with \texttt{ckertype} and \texttt{ckerorder} not being
  propagated in \texttt{np.singleindex.bw.R}

\item Fixed issue with negative penalties being returned by \texttt{bwmethod="cv.aic"} in \texttt{npregbw} (ought to have been +infinity)

\item Error being thrown by \texttt{system(..., intern=TRUE)} when \texttt{mpi.quit()} is called, changed to FALSE (change to \texttt{system()}
...
\item Fixed issue for \texttt{npRmpi} where the \texttt{C} code underlying
  regression cross-validation (code in \texttt{jksum.c}) differs between
...
\item Fixed a scope issue whereby a user would write a function that
  ...
  user's function and called by the \texttt{np/npRmpi} command (i.e. such
  as \texttt{newdata}) cannot be found in the environment yet they exist
  outside of the function

\item Fixed issue with \texttt{bwscaling=TRUE} and \texttt{bwmethod="cv.aic"} in npreg
...
\item Added asymptotic standard errors to \texttt{npindex} for the Klein
  and Spady and Ichimura parameter estimates which, when \texttt{gradients=TRUE}, can be extracted via \texttt{vcov(foo)} where \texttt{foo} is a npsingleindex object (the Z-scores can be obtained via
  \texttt{Z <- coef(foo)[-1]/sqrt(diag(vcov(foo)))[-1]})
...
\item Fixed code regression (dating to version 0.30-4) where \texttt{random.seed=42} was not initialized in functions \texttt{npcmstest},
  \texttt{npdeneqtest}, \texttt{npindexbw}, \texttt{npsdeptest}, \texttt{npqcmstest}, \texttt{npsigtest}, \texttt{npsymtest}, \texttt{npunitest},
  and \texttt{npplot}
...
\item Fixed minor i/o issue with \texttt{Multistart 1 of...} using \texttt{npscoefbw()}
...
\item Added a vignette and faq. To see the vignette try \texttt{vignette("np",package="np")}

\item \texttt{summary()} on \texttt{npconmode()} now properly retrieves
...
\item Fixed bug where \texttt{npindexbw()} would ignore \texttt{bandwidth.compute = FALSE} and compute bandwidths when supplied
