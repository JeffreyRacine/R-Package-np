...
\pkg{npRmpi} program, you can launch the script directly under
\proglang{MPI} and use \code{npRmpi.init(mode="attach")} inside your
script. No external bootstrap file is required. To
run the batch code in the file \code{npudensml_npRmpi.R} using two
processors on an
\proglang{MPI} system you will enter something like

\begin{verbatim}
mpiexec -n 2 Rscript --no-save npudensml_npRmpi.R
\end{verbatim}
...
\begin{verbatim}
sqsub -q mpi -n 2 R CMD BATCH --no-save npudensml_npRmpi.R
\end{verbatim}
...
For manual-broadcast/profile workflows (scripts that call
\code{mpi.bcast.cmd(...)} directly), use the \code{inst/Rprofile}
template via \code{R_PROFILE_USER} and launch with
\code{R CMD BATCH --no-save}. Do not use \code{--vanilla} for this
route because startup profiles are disabled.
...
with two minor differences. To start an interactive session, load
\pkg{npRmpi} and call \code{npRmpi.init(mode="spawn", nslaves=foo)}.
For example, on a dual-core desktop set \code{foo=1} so that one slave
is spawned in addition to the master node (two compute nodes total).
See the file \code{interactive_Rterm.R} in the demo directory for an
illustration.
...
## Start a spawned interactive session.
...
\begin{enumerate}
  ...
  \item Start and stop the MPI session using
    \code{npRmpi.init(mode="attach")} and \code{npRmpi.quit(mode="attach")}
    when running under \code{mpiexec}, or
    \code{npRmpi.init(mode="spawn", nslaves=...)} in interactive sessions.
    For manual-broadcast/profile scripts, use \code{inst/Rprofile}
    (\code{R_PROFILE_USER=...}) and explicit \code{mpi.bcast.*} calls.

  \item Enable auto-dispatch with
    \code{options(npRmpi.autodispatch=TRUE, np.messages=FALSE)} so
    standard \pkg{np} calls run across ranks without manual wrapping.

  \item For most estimators you can call \code{np*} functions directly
    (e.g., \code{npudensbw()}, \code{npregbw()}, \code{npcdensbw()}).

  \item Use direct master calls under
    \code{options(npRmpi.autodispatch=TRUE, np.messages=FALSE)}.
    In canonical SPMD mode, plotting calls (including bootstrap error
    bands) should be invoked as plain \code{plot(...)} from master.

  \item Nested \code{mpi.bcast.cmd(plot(...), caller.execute=TRUE)} is
    unsupported and fails fast by contract; keep \code{mpi.bcast.cmd}
    for low-level/manual-broadcast workflows, not top-level plot entry.
...
## mpiexec -n 2 Rscript --no-save npudensml_npRmpi.R
## (or mpiexec -n 2 R CMD BATCH --no-save npudensml_npRmpi.R).
## Check the time in the
...
## Initialize master/worker session in attach mode.
npRmpi.init(mode="attach", autodispatch=TRUE, np.messages=FALSE)

## Turn off progress i/o as this clutters the output file (if you want
## to see search progress you can comment out this command)

## (You can still set options() manually if preferred.)
