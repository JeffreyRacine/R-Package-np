...
\begin{enumerate}
  ...
  \item Bootstrap-error plotting is supported under autodispatch;
    explicit \code{mpi.bcast.cmd(plot(...), caller.execute=TRUE)}
    remains a useful option for heavy bootstrap workloads.
