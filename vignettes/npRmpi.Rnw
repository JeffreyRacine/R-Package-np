...
\begin{enumerate}
  ...
  \item Guarded exceptions still require explicit
    \code{mpi.bcast.cmd(..., caller.execute=TRUE)} (currently
    plotting calls with \code{plot.errors.method="bootstrap"}).

  \item Non-bootstrap plotting works directly with autodispatch.
    For bootstrap error bands use explicit
    \code{mpi.bcast.cmd(plot(...), caller.execute=TRUE)}.
