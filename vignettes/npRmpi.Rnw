...
\proglang{MPI} and use \code{npRmpi.start(mode="attach")} inside your
...
\begin{verbatim}
mpiexec -n 2 Rscript npudensml_npRmpi.R
\end{verbatim}
...
\pkg{npRmpi} and call \code{npRmpi.start(mode="spawn", nslaves=foo)}.
...
\begin{enumerate}
  ...
  \item Start and stop the MPI session using
    \code{npRmpi.start(mode="attach")} and \code{npRmpi.stop(mode="attach")}
    when running under \code{mpiexec}, or
    \code{npRmpi.start(mode="spawn", nslaves=...)} in interactive sessions.
...
## mpiexec -n 2 Rscript npudensml_npRmpi.R. Check the time in the
